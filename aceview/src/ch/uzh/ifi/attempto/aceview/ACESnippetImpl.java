...
/**
 * <p>
 * Every snippet is mapped to OWL/SWRL during construction time. The
 * corresponding OWL/SWRL axiom can be given as an argument in the constructor
 * (this is used by the verbalizer).
 * </p>
 * 
...
public class ACESnippetImpl implements ACESnippet {
	...
	/**
	 * <p>
	 * Constructs an ACE snippet from a single ACE sentence.
	 * </p>
	 * 
	 * @param ns
	 *            Default namespace of the snippet
	 * @param sentence
	 *            Sentence that the snippet contains
	 */
	...
	/**
	 * <p>
	 * Constructs an ACE snippet from a string. The corresponding (single) OWL axiom
	 * is given as input, therefore the snippet is not parsed. Note that any
	 * annotations (if present) are stripped from the axiom.
	 * </p>
	 * 
	 * @param ns
	 *            Default namespace of the snippet
	 ...
	 * @param axiom
	 *            OWL axiom that the snippet corresponds to
	 * @param altRendering
	 *            alternative rendering (MOS syntax) for the snippet
	 */
	...
	private void init() {
		...
		if (prefs.getUseMos() && sentences.size() == 1) {
			...
			try {
				...
				logger.debug("Parsing with the MOS parser: " + sentences.iterator().next());
				mosAxiom = ACETextManager.getInstance().parseWithMos(sentences.iterator().next(), base);
				logger.debug(mosAxiom);
	...
	private void parse(ACEViewPreferences prefs) throws OWLOntologyCreationException {
		TokenMapper aceLexicon = ACETextManager.getInstance().getACELexicon(ns);
	...
	private void parseWithAceParser(ACEViewPreferences prefs, TokenMapper aceLexicon, Set<String> contentWordforms) throws OWLOntologyCreationException {
		logger.debug("Wordforms: " + contentWordforms);
		Lexicon lexicon = aceLexicon.createLexicon(contentWordforms);
		if (lexicon.getEntries().isEmpty()) {
			logger.debug("Parsing with empty lexicon.");
			lexicon = null;
		} else {
			logger.debug("Parsing with lexicon:\n" + lexicon.toString());
		}
