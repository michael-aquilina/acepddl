 ...
 */
public final class ACETextManagerProtege implements IACETextManager {
	private final Logger logger = Logger.getLogger(ACETextManagerProtege.class);

	// changed IRI for Protege 5.2 as # was causing malformed error to be thrown
	private final IRI acetextIRI = IRI.create("http://attempto.ifi.uzh.ch/acetext"); 
	...
	private final IRI timestampIRI = IRI.create("http://purl.org/dc/elements/1.1/date");

	private final Map<OWLOntologyID, ACEText<OWLEntity, OWLLogicalAxiom>> acetexts = Maps.newHashMap();
	private final Map<OWLOntologyID, TokenMapper> acelexicons = Maps.newHashMap();
	private OWLModelManager owlModelManager;

	// BUG: maybe we should get a new instance whenever we need to query the
	...
	private ACESnippet selectedSnippet;

	// One snippet can be singled out with the "Why?"-button.
	// In this case, whySnippet refers to it.
	private ACESnippet whySnippet;

	private final List<ACEViewListener<ACEViewEvent<TextEventType>>> aceTextManagerChangeListeners = Lists.newArrayList();
	private final List<ACEViewListener<ACEViewEvent<SnippetEventType>>> snippetListeners = Lists.newArrayList();

	private boolean isInitCompleted = false;

	public ACETextManagerProtege() {
	}

	/**
	 * <p>
	 * If <code>acetiveACETextID == null</code> then it means that no ACE text has
	 * been created yet as one ACE text must always be active. In this case we
	 * create a new ACE text and set it active. Otherwise we change the active ACE
	 * text according to the given ID.
	 * </p>
	 * 
	 * @param id
	 */
	...
	/**
	 * <p>
	 * Returns the ACE lexicon (TokenMapper) that decides the surface forms of the
	 * snippets in this text.
	 * </p>
	 * 
	 * @param id
	 * @return Lexicon
	...
	/**
	 * <p>
	 * Adds the given snippet to the active ACE text, and adds the axioms of the
	 * snippet to the ontology that corresponds to the ACE text.
	 * </p>
	 * 
	 * @param snippet
	 *            ACE snippet
	...
	/**
	 * <p>
	 * Removes the given snippet from the active ACE text, and removes the axioms of
	 * the snippet from the ontology that corresponds to the ACE text.
	 * </p>
	 * 
	 * @param snippet
	 *            ACE snippet
	...
	/**
	 * <p>
	 * Updates the given snippet in the active text at the given index by first
	 * removing the snippet, then creating a new snippet out of the set of given
	 * sentences, and then adding the new snippet to the text and setting it as the
	 * selected snippet.
	 * </p>
	 * 
	 * @param index
	 *            Index of the snippet in the ACE text
	 * @param snippet
	 *            Snippet to be updated (i.e replaced)
	 * @param sentences
	 *            Sentences that form the new snippet
	 */
	@Override
	...
	/**
	 * <p>
	 * Adds a collection of ACE sentences and removes another collection of ACE
	 * sentences to/from the active ACE text.
	 * </p>
	 * 
	 * @param addedSentences
	 *            Collection of ACE sentences
	 * @param removedSentences
	 *            Collection of ACE sentences
	 */
	@Override
	...
	/**
	 * <p>
	 * Creates a new OWL ontology manager, but uses an existing OWL data factory.
	 * </p>
	 * 
	...
	public OWLOntologyManager createOWLOntologyManager() {
		...
		return owlModelManager.getOWLOntologyManager(); // changed from
														// OWLManager.createOWLOntologyManager(owlModelManager.getOWLDataFactory());
	}
	 ...
	 * 
	 *             <p>
	 *             Finds (a single) OWL entity based on the <code>EntryType</code>
	 *             and a lemma of an ACE word.
	 *             </p>
	 * 
	 *             TODO: "lemma of an ACE word" should really be "IRI of an OWL
	 *             entity"!
	 * 
	 *             FIXED: now using "false" in the getMatching*() calls. We are
	 *             interested in an exact match and not a prefix or regexp match.
	 *             Note that <code>getEntities(String)</code> does either wildcard
	 *             or regexp matching, depending on the preferences. Therefore, we
	 *             should escape all the wildcard symbols in the content words
	 *             before we start matching. Maybe there is a less powerful entity
	 *             finder somewhere, we don't really need regexp support when
	 *             clicking on the words.
	 * 
	 *             TODO: Get rid of this method. It is only used by
	 *             WordsHyperlinkListener, which we should also remove, and replace
	 *             it with a view which can hold the entities and thus does not have
	 *             search them via some string-based encoding (which is slow).
	 *
	 * @param type
	 *            Type (word class) of the lemma
	 * @param lemma
	 *            Lemma of a word
	...
	/**
	 * <p>
	 * Returns the entity that matches the IRI and the ACE word class (CN, TV, PN).
	 * </p>
	 * 
	 * TODO: Think about object and data properties. If TV corresponds to both then
	 * this method has to deal with the ambiguity.
	 * 
	 * @param type
	 *            ACE word class (CN, TV, PN)
	 * @param iri
	 *            OWL entity IRI
	...
	/**
	 * <p>
	 * Creates an ACE snippet from the given OWL axioms, and selects the created
	 * snippet.
	 * </p>
	 *
	 * @param axiom
	...
	/**
	 * <p>
	 * Selects the given snippet. Note that if the given snippet is already
	 * selected, then it is reselected. This is needed in order to refresh the
	 * views, if the only change was in terms of axiom annotations added to the
	 * snippet's axiom.
	 * </p>
	 * 
	 * @param snippet
	...
	/**
	 * <p>
	 * Remove the set of given logical axioms from the ontology, i.e. generate the
	 * respective list of changes. Note that the removed axioms do not have to match
	 * structurally against the given axioms. It is only important that the logical
	 * part matches, i.e. annotations are ignored.
	 * </p>
	 * 
	 * @param ont
	 *            OWL ontology to be modified
	 * @param axioms
	 *            Set of logical axioms to be removed
	...
	/**
	 * <p>
	 * Converts the given OWL logical axiom into its corresponding ACE snippet.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Returns a list of annotations that annotate the logical axioms of the given
	 * snippet.
	 * </p>
	 * 
	 * TODO: Why do we return a list? Because it is simpler to update a table model
	 * in this way
	 * 
	 * @param snippet
	 *            ACE snippet
	 ...
	 * 
	 *             <p>
	 *             Returns a list of annotations that annotate the logical axioms of
	 *             the given snippet. Only the ACE text annotation is not returned
	 *             because this is already explicitly present in the snippet.
	 *             </p>
	 * 
	 *             TODO: Why do we return a list? Because it is simpler to update a
	 *             table model in this way
	 * 
	 * @param snippet
	 *            ACE snippet
		...
		for (OWLLogicalAxiom ax : snippet.getLogicalAxioms()) {
			...
			for (OWLAxiom ax1 : getOWLModelManager().getActiveOntology().getAxiomsIgnoreAnnotations(ax)) {
				for (OWLAnnotation annotation : ax1.getAnnotations()) {
					if (!annotation.getProperty().getIRI().equals(this.acetextIRI)) {
	 ...
	 * 
	 *             <p>
	 *             Returns a list of changes that would remove all the annotations
	 *             from the given ontology, that annotate the given entity and have
	 *             the given IRI as the annotation IRI.
	 *             </p>
	 * 
	 * @param ont
	 *            OWL ontology
	 ...
	 */
	private List<RemoveAxiomByACEView> findEntityAnnotationAxioms(OWLOntology ont, OWLEntity entity, IRI iri) {
		List<RemoveAxiomByACEView> axioms = Lists.newArrayList();
		for (OWLAnnotationAssertionAxiom axiom : EntitySearcher.getAnnotationAssertionAxioms(entity, ont)) {
	...
	/**
	 * <p>
	 * Returns the rendering of the entity, as decided by the current renderer in
	 * the model manager.
	 * </p>
	 * <p>
	 * The renderer adds quotes around strings that contains spaces (e.g. a label
	 * like "Eesti Vabariik"). We remove such quotes, otherwise it would confuse the
	 * sorter.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Interprets the given ACE sentence as an expression in Manchester OWL Syntax
	 * and parses it to an OWL logical axiom.
	 * </p>
	 * 
	 * @param sentence
	 * @param base
	 ...
	 */
	@Override
	public OWLLogicalAxiom parseWithMos(ACESentence sentence, String base) {
		// Remove the last token (a dot or a question mark) of the given sentence.
		String mosStr = sentence.toMOSString();
		OWLModelManager mngr = getOWLModelManager();
		return OntologyUtils.parseWithMosParser(mngr.getOWLOntologyManager(), mngr.getActiveOntology(), mngr.getOWLDataFactory(), new ProtegeOWLEntityChecker(mngr.getOWLEntityFinder()), base, mosStr, sentence.isQuestion());
	}

	/**
	 * <p>
	 * Adds the given snippet to the given ACE text, and adds the axioms of the
	 * snippet to the ontology that corresponds to the ACE text.
	 * </p>
	 * 
	 * @param acetext
	 ...
	 *            ACE snippet
	 */
	private void addSnippet(ACEText<OWLEntity, OWLLogicalAxiom> acetext, ACESnippet snippet) {
		acetext.add(snippet);
	...
	private void changeOntology(List<? extends OWLAxiomChange> changes) {
	...
	private List<OWLAxiomChange> findAndRemove(ACESentence sentence) {
		ACEText<OWLEntity, OWLLogicalAxiom> acetext = getActiveACEText();
		OWLOntology ontology = owlModelManager.getActiveOntology();
		List<OWLAxiomChange> changes = Lists.newArrayList();
	...
	private String getHtmlHead() {
		...
		return "<style type='text/css'>" + "	body { font-size: " + fontSize + "; font-family: " + fontName + "; margin-left: 4px; margin-right: 4px; margin-top: 4px; margin-bottom: 4px; background-color: #ffffee }" + "	table { border-width: 1px; border-style: solid; border-color: silver; empty-cells: show; border-collapse: collapse; margin-bottom: 1em }"
				+ "	td { border-width: 1px; border-style: solid; border-color: silver }" + "	div { padding-left: 4px; padding-right: 4px; padding-top: 4px; padding-bottom: 4px;" + "		margin-left: 4px; margin-right: 4px; margin-top: 4px; margin-bottom: 4px }" + "	div.messages { border-width: 3px; border-color: red  }" + "	p.question { color: olive }" + "	.indent { margin-left: 15px }"
				+ "	.error { color: red }" + "	a { text-decoration: none }" + "</style>";
	}
	...
	/**
	 * TODO: Currently we can only store the text and timestamp of the snippet if
	 * the snippet corresponds to a single axiom. Would be nice if a group of axioms
	 * could be annotated as well.
	 * 
	 * @param ontology
	 *            OWL ontology
	 * @param snippet
	 *            ACE snippet
	 */
	private List<? extends OWLAxiomChange> getAddChanges(OWLOntology ontology, ACESnippet snippet) {
		List<AddAxiomByACEView> changes = Lists.newArrayList();
	...
	/**
	 * <p>
	 * Note that we do not add the axiom into the ontology, because we expect it to
	 * be there already, as it is one of the tangling axioms.
	 * </p>
	 * 
	 * @param acetext
	 *            ACE text
	 ...
	 *            OWL axiom
	 */
	private void verbalizeAndAdd(ACEText<OWLEntity, OWLLogicalAxiom> acetext, OWLOntology ont, AxiomVerbalizer axiomVerbalizer, OWLLogicalAxiom axiom) {
		...
		if (snippet != null) {
			 ...
			 * OWLAxiomAnnotationAxiom annAcetext = OntologyUtils.createAxiomAnnotation(df,
			 * axiom, acetextIRI, snippet.toString()); OWLAxiomAnnotationAxiom annTimestamp
			 * = OntologyUtils.createAxiomAnnotation(df, axiom, timestampURI,
			 * snippet.getTimestamp().toString()); List<OWLAxiomChange> changes =
			 * Lists.newArrayList(); changes.add(new AddAxiomByACEView(ont, annAcetext));
	...
	private AxiomVerbalizer createAxiomVerbalizer() {
		return new AxiomVerbalizer(new VerbalizerWebservice(ACEViewPreferences.getInstance().getOwlToAce()));
	}

	/**
	 * <p>
	 * Creates a new snippet from the given OWL axiom, and adds the snippet to the
	 * given ACE text. See also {@link #addSnippet(ACEText, ACESnippet)}.
	 * </p>
	 * 
	 * @param acetext
	 *            ACE text
	 ...
	 */
	/*
	 * private void addAxiom(ACEText<OWLEntity, OWLLogicalAxiom> acetext,
	 * OWLLogicalAxiom axiom) { AxiomVerbalizer axiomVerbalizer =
	 * createAxiomVerbalizer(acetext.getACELexicon()); OWLModelManager mm =
	 * getOWLModelManager(); OWLOntology ont = mm.getActiveOntology(); ACESnippet
	 * snippet = null; try { snippet = axiomVerbalizer.verbalizeAxiom(ont.getURI(),
	 * axiom); addSnippet(acetext, snippet); } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */
	 ...
	 * 
	 *             <p>
	 *             Constructs a new axiom on the basis of the given axiom. The new
	 *             axiom will have some additional annotations based on the given
	 *             snippet, namely the textual content of the snippet and the
	 ...
	 *            OWLDataFactory
	 * @param axiom
	 *            Axiom to be annotated
	 * @param snippet
	 *            Snippet that provides the content of the annotations
	...
	private void fireSnippetEvent(SnippetEventType type) {
	...
	/**
	 * <p>
	 * Returns the id (OWLOntologyID) of the active ACE text. In case no ACE text
	 * has been set as active then returns <code>null</code>.
	 * </p>
	 * 
	 * @return ID of the active ACE text.
	 */
	private OWLOntologyID getActiveID() {
