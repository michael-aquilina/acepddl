...
/**
 * <p>
 * The ACE View tab monitors several Protege events and generates corresponding
 * ACE View events.
 * </p>
 * 
...
public class ACEViewTab extends OWLWorkspaceViewsTab {
	...
	private final OWLModelManagerListener modelManagerListener = new OWLModelManagerListener() {
		public void handleChange(OWLModelManagerChangeEvent event) {
			...
			if (event.isType(org.protege.editor.owl.model.event.EventType.ACTIVE_ONTOLOGY_CHANGED)) {
				ACETextManager.getInstance().setActiveACETextID(getOWLModelManager().getActiveOntology().getOntologyID());
			} else if (event.isType(org.protege.editor.owl.model.event.EventType.ONTOLOGY_LOADED)) {
				// TODO: remove:
				// ACETextManager.getInstance().createACEText(getOWLModelManager().getActiveOntology().getOntologyID());
				try {
					initACEText();
					// TODO: BUG: nothing is monitoring this event
					// ACETextManager.getInstance().fireEvent(EventType.ACETEXT_LOADED);
				} catch (Exception e) {
				...
				if (ACEViewPreferences.getInstance().isUpdateAnswersOnClassify()) {
					new UpdateAnswersUI(null, ACETextManager.getInstance().getActiveACEText(), getOWLModelManager()).updateAnswers();
				}
			} else if (event.isType(org.protege.editor.owl.model.event.EventType.ONTOLOGY_CREATED)) {
				// TODO: maybe the ACETEXT_CREATED even is not needed anymore (delete it from
				// everywhere)
				// If an ontology is created then this ontology is set as active, therefore
				// we can get the URI of this ontology by asking for the URI of the active
				// ontology.
				// ACETextManager.getInstance().createACEText(getOWLModelManager().getActiveOntology().getOntologyID());
				// ACETextManager.getInstance().fireEvent(EventType.ACETEXT_CREATED);
			} else {
	...
	// this is fired 4 times (for Class, OProp, DProp, NamedInd), which leads to
	// complete
	...
	private final OWLEntityRendererListener entityRendererListener = new OWLEntityRendererListener() {
		public void renderingChanged(OWLEntity entity, OWLModelManagerEntityRenderer renderer) {
			...
			logger.info("Rendering for " + entity + " with type " + entity.getEntityType() + " changed to "
					+ entityRendering);

			// TODO: BUG: only add morph annotations if prefs.isUseLexicon == true
			/*
			 * OWLModelManager mm = getOWLModelManager(); OWLOntology ont =
			 * mm.getActiveOntology();
			 * 
			 * List<OWLAxiomChange> changeList1 = removeMorfAnnotations(ont, entity);
			 * List<OWLAxiomChange> changeList2 = addMorfAnnotations(mm.getOWLDataFactory(),
			 * ont, entity, entityRendering); changeList1.addAll(changeList2);
			 * 
	...
	public void initialise() {
		...
		acetextAnnProp = mm.getOWLDataFactory().getOWLAnnotationProperty(ACETextManager.getInstance().getAcetextIRI());

		ACETextManager.getInstance().setOWLModelManager(mm);
		// TODO: remove:
		// ACETextManager.getInstance().createACEText(mm.getActiveOntology().getOntologyID());
	...
	/**
	 * <p>
	 * This method is called either when the ACE View tab is launched, or when the
	 * ONTOLOGY_LOADED even is fired. In both cases the ACE text must be generated
	 * that corresponds to the ontology.
	 * </p>
	 * 
	 ...
	 */
	private static void initACEText()
			throws OWLRendererException, OWLOntologyCreationException, OWLOntologyChangeException {
		OWLModelManager mm = ACETextManager.getInstance().getOWLModelManager();
		...
		ACETextManager.getInstance().setInitCompleted(false);
		for (OWLOntology ont : ontologies) {
			logger.info("Init: ontology:\n" + getOntologyInfo(ont));
			initACETextFromOntology(mngr, df, prefs, ont);
		}
		ACETextManager.getInstance().setInitCompleted(true);
		ACETextManager.getInstance().fireEvent(TextEventType.ACETEXT_LOADED);
	}

	/**
	 * Note that this method is called only when a new ontology is loaded.
	 * 
	 * Translate every OWL logical axiom into ACE snippet and add it to the ACE
	 * text. If the logical axiom is already annotated with an ACE snippet then add
	 * the snippet instead.
	 * 
	 * @param mngr
	 * @param df
	 ...
	 * @throws OWLOntologyChangeException
	 */
	private static void initACETextFromOntology(OWLOntologyManager mngr, OWLDataFactory df, ACEViewPreferences prefs,
			OWLOntology ont) throws OWLRendererException, OWLOntologyCreationException, OWLOntologyChangeException {

		OWLOntologyID id = ont.getOntologyID();
		ACEText<OWLEntity, OWLLogicalAxiom> acetext = ACETextManager.getInstance().getACEText(id);
		TokenMapper tokenMapper = ACETextManager.getInstance().getACELexicon(id);
		if (prefs.isUseLexicon()) {
			...
			for (OWLEntity entity : ont.getSignature()) {
				if (Showing.isShow(entity)) {
					...
					String lemma = ACETextManager.getInstance().getRendering(entity);
	...
	/**
	 * <p>
	 * This is called when a new ontology is loaded. This ontology can already
	 * contain morphological annotations (i.e. we have to add the corresponding
	 * lexicon entries). In addition we add the morphological annotations which the
	 * ontology does not yet contain. Their corresponding lexicon entries will be
	 * generated by the annotation axiom monitor.
	 * </p>
	 * 
	 * <p>
	 * For each entity: add morphological annotations (CN_sg, CN_pl, TV_sg, TV_pl,
	 * TV_vbg, PN_sg) to the ontology.
	 * </p>
	 * 
	 ...
	 * @param entities
	 */
	private static void addMorfAnnotations(OWLOntologyManager ontologyManager, OWLDataFactory df, OWLOntology ont,
			Set<OWLEntity> entities, TokenMapper tokenMapper) {
		...
		for (OWLEntity entity : entities) {
			if (Showing.isShow(entity)) {
				...
				Set<OWLAnnotationAssertionAxiom> annSet = MorphAnnotation.getAdditionalMorphAnnotations(df, ont,
						entity);
	...
	/**
	 * <p>
	 * Monitors all the changes that happen in the model and reacts to them, e.g.
	 * updates the ACE text. Only axiom changes (additions, removals) are handled.
	 * The respective axiom can be a logical or an annotation axiom.
	 * </p>
	 * 
	 * @param changes
	 *            List of ontology changes
	 */
	private static void processChanges(OWLModelManager mngr, List<? extends OWLOntologyChange> changes) {
		...
		for (OWLOntologyChange change : changes) {
			...
			TokenMapper acelexicon = ACETextManager.getInstance().getACELexicon(oid);
			...
			if (axiom instanceof OWLLogicalAxiom) {
				...
				if (change instanceof AddAxiomByACEView) {
					...
					// entity annotations) are not sent to the parser at all, they are rejected
					// before that.
					continue;
				}
					...
					textAxiomCounter++;
					Set<OWLEntity> entities = ((OWLAxiomChange) change). getSignature();
					...
					ACETextManager.getInstance().processTanglingAxioms(acetext, tanglingAxioms);
				} else {
				...
				if (prefs.isUseLexicon()) {
					...
					if (change instanceof AddAxiom) {
						...
						Set<OWLAnnotationAssertionAxiom> morphAnnotations = MorphAnnotation
								.getAdditionalMorphAnnotations(df, changeOnt, entity);
						logger.info("Triggered: add: " + morphAnnotations);
						ACETextManager.getInstance().addAxiomsToOntology(ontologyManager, changeOnt, morphAnnotations);
					} else if (change instanceof RemoveAxiom) {
		...
		if (textAxiomCounter > 0) {
			ACETextManager.getInstance().fireEvent(TextEventType.ACETEXT_CHANGED);
		}
		if (lexiconAxiomCounter > 0) {
			ACETextManager.getInstance().fireEvent(TextEventType.ACELEXICON_CHANGED);
		}
	}

	/**
	 * <p>
	 * Processes a logical axiom that has been added by Protege when loading an
	 * ontology. The input axiom undergoes the following processing.
	 * </p>
	 * 
	 * <ol>
	 * <li>Does it contain any ACE annotations (i.e. has this axiom been already
	 * ACEified)?
	 * <ol>
	 * <li>If yes then convert the annotations (there can be more than one) into ACE
	 * snippets and add the snippets to the ACE text</li>
	 * <li>If not then
	 * <ol>
	 ...
	 * @throws OWLOntologyChangeException
	 */
	private static void processAxiomOnLoad(OWLOntology ont, ACEText acetext, AxiomVerbalizer axiomVerbalizer,
			OWLOntologyID ns, OWLLogicalAxiom logicalAxiom)
			throws OWLRendererException, OWLOntologyCreationException, OWLOntologyChangeException {
	...
	/**
	 * <p>
	 * Processes a logical axiom that has been added by Protege, either via the GUI
	 * or when loading an ontology. The input axiom undergoes the following
	 * processing.
	 * </p>
	 * 
	 * <ol>
	 ...
	 * <li>renew the axiom in the ontology (i.e. remove it and add it back with the
	 * annotation), triggering a new AddAxiom-event which will be picked up by case
	 * 1.</li>
	 * </ol>
	 * </li>
	 * </ol>
	 ...
	 * @throws OWLOntologyChangeException
	 */
	private static void processAxiom(OWLOntology ont, OWLDataFactory df, OWLOntologyManager mngr, ACEText acetext,
			AxiomVerbalizer axiomVerbalizer, OWLOntologyID ns, OWLLogicalAxiom logicalAxiom)
			throws OWLRendererException, OWLOntologyCreationException, OWLOntologyChangeException {
		...
		for (OWLAnnotation annotation : logicalAxiom.getAnnotations(acetextAnnProp)) {
			...
			if (aceAnnotationValue == null) {
				...
				newSnippet = new ACESnippetImpl(ns, aceAnnotationValue, logicalAxiom);
				acetext.add(newSnippet);
				ACETextManager.getInstance().setSelectedSnippet(newSnippet);
			}
		}
		...
		if (newSnippet == null) {
			...
			if (newSnippet == null) {
				...
				// TODO: delete this
				/*
				 * // TODO: use instead?: List<OWLOntologyChange> changes = new
				 * ArrayList<OWLOntologyChange>(); List<OWLAxiomChange> changes =
				 * Lists.newArrayList(); // Remove the axiom from the ontology changes.add(new
				 * RemoveAxiom(ont, logicalAxiom)); // Add back the axiom, but now with the
				 * additional annotation. changes.add(new AddAxiom(ont,
				 * ACETextManager.getInstance().annotateAxiomWithSnippet(df, logicalAxiom, newSnippet))); //
				 * Apply the changes to the ontology OntologyUtils.changeOntology(mngr,
				 * changes);
				 */
			}
		}
		...
		// the name of the other property. This way the verbalization comes out nicer,
		// e.g.
		...
		// 2. the lexicon might not like 'is-larger-than' used both as sg and vbg (in
		// different entries)
		// TODO: This if-sentence is currently switched off.
		// if (logicalAxiom instanceof OWLInverseObjectPropertiesAxiom) {
		// OWLInverseObjectPropertiesAxiom iopa = (OWLInverseObjectPropertiesAxiom)
		// logicalAxiom;
		// if (! iopa.getFirstProperty().isAnonymous() && !
		// iopa.getSecondProperty().isAnonymous()) {
		...
		// ACETextManager.getInstance().getRemoveChanges(ont, p1.getAnnotationAssertionAxioms(ont)));
		// Set<OWLAxiom> set = Sets.newHashSet();
		// set.add(getTV_vbg(df, p1, p2));
		// set.add(getTV_vbg(df, p2, p1));
		// ACETextManager.getInstance().addAxiomsToOntology(mngr, ont, set);
		// }
		// }
	}
	...
	/**
	 * <p>
	 * The annotation value can be either a literal or an anonymous individual. We
	 * are interested only in literals and only in their lexical value, and not the
	 * lang-attribute nor the datatype. If the annotation value is not a literal
	 ...
	 */
	// private static OWLAnnotationAssertionAxiom getTV_vbg(OWLDataFactory df,
	...
	// return OntologyUtils.createIRIAnnotationAxiom(df, MorphType.TV_VBG.getIRI(),
	// p1.getIRI(), vbgForm);
	// }

	private static List<OWLAxiomChange> addMorfAnnotations(OWLDataFactory df, OWLOntology ont, OWLEntity entity,
			String lemma) {
	...
	/**
	 * <p>
	 * Removes all the morph. annotation axioms that apply to the IRI of the given
	 * entity.
	 * </p>
	 * 
	 * @param ont
	...
	private static List<OWLAxiomChange> removeMorfAnnotations(OWLOntology ont, OWLEntity entity) {
		...
		for (OWLAnnotationAssertionAxiom ax : EntitySearcher.getAnnotationAssertionAxioms(entity,ont)) {
	...
	/**
	 * <p>
	 * Updates the lexicon on the basis of the entity but applies very simple
	 * morphological synthesis (= every wordform is identical to the Protege
	 * rendering of the entity, e.g. singular and plural forms are identical). Also,
	 * the morph. annotation axioms are not generated.
	 * </p>
	 * 
	 * @param entity
	 * @param tokenMapper
	...
	private static void updateLexiconFromExistingAnnotations(OWLEntity entity, OWLOntology ont,
	...
	/**
	 * <p>
	 * Prints some data about the ontology. Useful for debugging.
	 * </p>
	 * 
	 * @param ont
	 * @return
	...
	private static String getOntologyInfo(OWLOntology ont) {
		...
		sb.append("Entities (here): " + ont.getSignature().size() + "\n");
		sb.append("Entities (here + imported): " + ont.getSignature().size() + "\n");
		return sb.toString();
	}
}
