...
public class ACEAnswersPane extends JTextPane {
	...
	private void showNothingSnippet(OWLClassExpression dlquery) {
		...
		try {
			...
			if (nothingSnippet == null) {
				nothingSnippet = ACETextManager.getInstance().makeSnippetFromAxiom(axiom);
			...
			buttonWhy.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent event) {
					ACETextManager.getInstance().setWhySnippet(finalNothingSnippet);
				}
			});
			addLinebreak();
			...
			addComponent(new JLabel("<html>" + nothingSnippet.toHtmlString(ACETextManager.getInstance().getActiveACELexicon()) + "</html>"));
			addLinebreak();
	...
	private boolean confirmAndAdd(final JButton buttonCompleter, OWLLogicalAxiom axiom) {
		...
		try {
			snippet = ACETextManager.getInstance().makeSnippetFromAxiom(axiom);
		} catch (OWLRendererException e) {
			e.printStackTrace();
		} catch (OWLOntologyCreationException e) {
			e.printStackTrace();
		...
		if (snippet != null) {
			...
			if (ret == JOptionPane.OK_OPTION) {
				...
				ACETextManager.getInstance().addSnippet(snippet);
