...
public class AnnotateAxiomsWithACETextAction extends ProtegeOWLAction {
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		for (OWLOntology ont : ontologies) {
			...
			ontologyCount ++;
			ACEText<?, OWLLogicalAxiom> acetext = ACETextManager.getInstance().getACEText(ont.getOntologyID());
			for (OWLLogicalAxiom axiom : ont.getLogicalAxioms()) {
				...
				if (snippets.size() == 1) {
					...
					OWLLogicalAxiom newAxiom = ACETextManager.getInstance().annotateAxiomWithSnippet(df, axiom, snippet);
