...
public class ACEEntailmentsViewComponent extends AbstractACESnippetsViewComponent {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					if (row != -1) {
						...
						ACETextManager.getInstance().setWhySnippet(snippet);
					}
				}
			}
		});
	...
	private void getInferredSentences() throws OWLOntologyCreationException, OWLOntologyChangeException, OWLRendererException {
		...
		OWLOntologyManager ontologyManager = ACETextManager.getInstance().createOWLOntologyManager();
		// Creates a new empty ontology.
		OWLOntology inferredOnt = OntologyUtils.createOntology(ontologyManager);
		ontGen.fillOntology(ontologyManager.getOWLDataFactory(), inferredOnt);
		logger.info("Done.");
		...
		TokenMapper lexicon = ACETextManager.getInstance().getActiveACELexicon();
