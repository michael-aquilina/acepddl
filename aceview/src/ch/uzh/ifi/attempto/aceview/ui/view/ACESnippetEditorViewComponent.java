...
public class ACESnippetEditorViewComponent extends AbstractACESnippetSelectionViewComponent {
	...
	private final ACEViewListener<ACEViewEvent<TextEventType>> aceTextManagerListener = new ACEViewListener<ACEViewEvent<TextEventType>>() {
		public void handleChange(ACEViewEvent<TextEventType> event) {
			...
			if (event.isType(TextEventType.ACTIVE_ACETEXT_CHANGED) ||
				...
				snippetEditor.setAutocompleter(ACETextManager.getInstance().getActiveACELexicon().getAutocompleter());
			}
		}
	};
	...
	protected void initialiseOWLView() throws Exception {
		...
		buttonNew.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if (sentences.isEmpty()) {
					...
					OWLOntologyID oid = ACETextManager.getInstance().getOWLModelManager().getActiveOntology().getOntologyID();
		...
		buttonUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				ACESnippet selectedSnippet = ACETextManager.getInstance().getSelectedSnippet();
				if (selectedSnippet != null) {
					...
					if (sentences.isEmpty()) {
						...
						ACESnippet oldSnippet = acetext.find(sentences);
						if (oldSnippet == null) {
							ACETextManager.getInstance().updateSnippet(acetext.indexOf(selectedSnippet), selectedSnippet, sentences);
						}
						else {
		...
		buttonWhy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				ACESnippet selectedSnippet = ACETextManager.getInstance().getSelectedSnippet();
				if (selectedSnippet != null) {
					ACETextManager.getInstance().setWhySnippet(selectedSnippet);
				}
			}
		});
		...
		ACETextManager.getInstance().addSnippetListener(getACESnippetListener());
	...
	protected void disposeOWLView() {
		...
		ACETextManager.getInstance().removeSnippetListener(getACESnippetListener());
	}
	...
	protected void displaySnippet(ACESnippet snippet) {
		...
		if (ACETextManager.getInstance().getActiveACEText().contains(snippet)) {
	...
	private void deleteSnippet(ACESnippet snippet) {
		ACETextManager.getInstance().removeSnippet(snippet);
		ACETextManager.getInstance().resetSelectedSnippet();
