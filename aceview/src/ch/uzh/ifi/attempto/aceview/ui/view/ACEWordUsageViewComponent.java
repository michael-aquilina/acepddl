...
/**
 * <p>
 * Shows the selected entity, its rendering and annotations, and a list of ACE
 * snippets that reference this entity.
 * </p>
 * 
...
public class ACEWordUsageViewComponent extends AbstractACEViewComponent {
	...
	public void initialiseView() throws Exception {
		editorpaneIndex = new JEditorPane("text/html", ACETextManager.getInstance().wrapInHtml(""));
		...
		JScrollPane scrollpaneIndex = new JScrollPane(editorpaneIndex, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	protected OWLObject updateView() {
		...
		if (isShowing()) {
			if (entity != null && Showing.isShow(entity)) {
				...
				ACEText<OWLEntity, ?> acetext = ACETextManager.getInstance().getACEText(oid);
				TokenMapper tokenMapper = ACETextManager.getInstance().getACELexicon(oid);
				...
				editorpaneIndex.setText(ACETextManager.getInstance().wrapInHtml(entityAnnotations + thereAreXSnippets + indexEntry));
			} else {
				editorpaneIndex.setText(ACETextManager.getInstance().wrapInHtml(""));
			}
		}
		return entity;
	}
