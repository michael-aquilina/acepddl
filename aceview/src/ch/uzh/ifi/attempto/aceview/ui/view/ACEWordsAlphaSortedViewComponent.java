...
public class ACEWordsAlphaSortedViewComponent extends AbstractACEViewComponent {
	...
	private final OWLModelManagerListener modelManagerListener = new OWLModelManagerListener() {
		public void handleChange(OWLModelManagerChangeEvent event) {
			if (event.isType(org.protege.editor.owl.model.event.EventType.ENTITY_RENDERER_CHANGED)) {
				editorpaneWords.setText(ACETextManager.getInstance().wrapInHtml(getContentWordsInHtml(ACETextManager.getInstance().getActiveACEText())));
			}
		}
	};
	...
	public void initialiseView() throws Exception {
		...
		editorpaneWords = new JEditorPane("text/html", ACETextManager.getInstance().wrapInHtml(""));
	...
	private void showWords() {
		...
		if (contentWordCount > 0) {
			editorpaneWords.setText(ACETextManager.getInstance().wrapInHtml(getContentWordsInHtml(acetext)));
		}
		else {
			editorpaneWords.setText(ACETextManager.getInstance().wrapInHtml(""));
		}
	}
	...
	private String getContentWordsInHtml(ACEText<OWLEntity, OWLLogicalAxiom> acetext) {
		...
		for (OWLEntity entity : entitiesSorted) {
			...
			String entityRendering = ACETextManager.getInstance().getRendering(entity);
