...
/**
 * This class is used to convert Java plan into its JSON representation. Its
 * also provide methods to save the JSON String into a file.
 *
...
public class AdapterPlanJavaJson
{
    ...
    public void saveInFile(String name)
    {
        ...
        try (Writer writer = new OutputStreamWriter(new FileOutputStream(name),
                "UTF-8"))
        {
...
//
//                // Preconditions
//                ArrayList<ArrayList<String>> preconds = this
//                        .toStringJ(action.getPreconditions());
...
//
//                    ArrayList<ArrayList<String>> condExpElementsCondition = this
//                            .toStringJ(condExp.get(k).getCondition());
//
//                    JSONArray positivesConditionJson = listToJson(
//                            condExpElementsCondition.get(0));
//                    JSONArray negativesConditionJson = listToJson(
//                            condExpElementsCondition.get(1));
//
//                    expJsonCondition.put("Positives", positivesConditionJson);
//                    expJsonCondition.put("Negatives", negativesConditionJson);
//
//                    ArrayList<ArrayList<String>> condExpElementsEffect = this
//                            .toStringJ(condExp.get(k).getEffects());
//
//                    JSONArray positivesEffectJson = listToJson(
//                            condExpElementsEffect.get(0));
//                    JSONArray negativesEffectJson = listToJson(
//                            condExpElementsEffect.get(1));
//
...
//        }
    ...
    {
        return AdapterPlanJavaJson.toStringJ(exp, codedProblem.getConstants(),
                codedProblem.getTypes(), codedProblem.getPredicates(),
                codedProblem.getFunctions(), codedProblem.getRelevantFacts());
    }

    private ArrayList<ArrayList<String>> toStringJ(BitOp exp)
    {
    	ArrayList<ArrayList<String>> list = new ArrayList<ArrayList<String>>() ;
    	ArrayList<String> innerList = new ArrayList<String>();
    	innerList.add("<ACEPlannerActionModel toStringJ not implemented>");
    	list.add(innerList);
    	return list;
    }
     ...
     * @return an 2D collection of Strings
     */
    private static ArrayList<ArrayList<String>> toStringJ(BitExp exp,
            ...
            final List<IntExp> relevants)
    {
        ...
        for (int i = positive.nextSetBit(0); i >= 0; i = positive
                .nextSetBit(i + 1))
        {
            fluentsPos.add(StringEncoder.toString(relevants.get(i), constants,
        ...
        for (int i = negative.nextSetBit(0); i >= 0; i = negative
                .nextSetBit(i + 1))
        {
            fluentsNeg.add(StringEncoder.toString(relevants.get(i), constants,
                    types, predicates, functions, " "));
        }
