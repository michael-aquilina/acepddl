...
/**
 * This class implements all the methods needed to encode operators, goal and
 * initial state loaded from the parser in its integer representation.
 *
 * @author D. Pellier
...
final class IntEncoding
{
    ...
    /**
     * Collects composite type, i.e., type of the form (either t1 t2), through a
     * specified domain and problem and creates their respective domain.
     *
        ...
        for (TypedSymbol elt : list)
        {
            ...
            if (types.size() > 1)
            {
                ...
                for (Symbol type : types)
                {
                    ...
                    final Set<Integer> typeDomain = Encoder.tableOfDomains
                            .get(typeIndex);
                ...
                if (index == -1)
                {
                    Encoder.tableOfDomains
                            .add(new LinkedHashSet<>(newTypeDomain));
    ...
    static void encodeConstants(final Domain domain, final Problem problem)
    {
        ...
        Encoder.tableOfConstants = new ArrayList<>(
                domain.getConstants().size());
        ...
        for (TypedSymbol constant : constants)
        {
            ...
            final LinkedList<Symbol> types = new LinkedList<>(
                    constant.getTypes());
            while (!types.isEmpty())
            {
    ...
    static void encodePredicates(final Domain domain)
    {
        ...
        for (NamedTypedList predicate : predicates)
        {
            ...
            for (TypedSymbol arg : arguments)
            {
                ...
                {
                    argType.add(Encoder.tableOfTypes
                            .indexOf(types.get(0).getImage()));
                }
            }
     ...
     *
     * @param list the list of operators to encode.
     * @return encoded the list of operators encoded.
     */
    static List<IntOp> encodeOperators(final List<ACEPlannerOp> list)
    {
        return list.stream().map(IntEncoding::encodeOperator)
    ...
    {
        return init.stream().map(IntEncoding::encodeExp)
    ...
    private static IntOp encodeOperator(final Op op)
    {
        ...
        for (int i = 0; i < op.getArity(); i++)
        {
            ...
            final String typeImage = IntEncoding
                    .toStringType(parameter.getTypes());
        ...
        final IntExp preconditions = IntEncoding
                .encodeExp(op.getPreconditions(), variables);
        intOp.setPreconditions(preconditions);
        // Encode the effects of the operator
        final IntExp effects = IntEncoding.encodeExp(op.getEffects(),
                variables);
    ...
    private static IntExp encodeExp(final Exp exp, final List<String> variables)
    {
        ...
        case EQUAL_ATOM:
            ...
            for (int i = 0; i < exp.getAtom().size(); i++)
            {
                ...
                {
                    args[i] = Encoder.tableOfConstants
                            .indexOf(argument.getImage());
                }
            }
        ...
        case FN_HEAD:
            ...
            for (int i = 1; i < exp.getAtom().size(); i++)
            {
                ...
                {
                    args[i - 1] = Encoder.tableOfConstants
                            .indexOf(argument.getImage());
                }
            }
            intExp.setArguments(args);
        ...
        case OR:
            ...
            {
                intExp.getChildren().add(IntEncoding
                        .encodeExp(exp.getChildren().get(i), variables));
            }
            break;
        case FORALL:
        case EXISTS:
            ...
            final String type = IntEncoding
                    .toStringType(qvar.get(0).getTypes());
            ...
            {
                intExp.getChildren().add(IntEncoding
                        .encodeExp(exp.getChildren().get(0), newVariables));
            }
            else
            {
                qvar.remove(0);
                intExp.getChildren()
                        .add(IntEncoding.encodeExp(exp, newVariables));
            }
            break;
        case F_EXP_T:
            if (!exp.getChildren().isEmpty())
            {
                intExp.getChildren().add(IntEncoding
                        .encodeExp(exp.getChildren().get(0), variables));
            }
            break;
        case FN_ATOM:
        case WHEN:
        ...
        case WITHIN:
        case HOLD_AFTER:
            intExp.getChildren().add(
                    IntEncoding.encodeExp(exp.getChildren().get(0), variables));
            intExp.getChildren().add(
                    IntEncoding.encodeExp(exp.getChildren().get(1), variables));
            break;
        case AT_START:
        case AT_END:
        case MINIMIZE:
        ...
        case HOLD_DURING:
            ...
            intExp.getChildren().add(
                    IntEncoding.encodeExp(exp.getChildren().get(1), variables));
            intExp.getChildren().add(
                    IntEncoding.encodeExp(exp.getChildren().get(2), variables));
            break;
        case TIME_VAR:
        case IS_VIOLATED:
            // Do nothing
