...
import fr.uga.pddl4j.util.IntExp;s
...
/**
 * This class contains the methods for the pre instantiation step. In other
 * words, it contains methods in order to:
 * <ul>
 * <li>extract inertia information from the operators encoded into integer
 * representation.</li>
 * <li>create predicates tables used to count the occurrences of a specified
 * predicates in the initial state.</li>
...
final class PreInstantiation
{
    ...
    private static final Logger LOGGER = LogManager
            .getLogger(PreInstantiation.class);
    ...
    /**
     * This method proceeds over the operators of the domain and checks for all
     * atom which kind of inertia it is. For each atom it checks if it satisfies
     ...
     * </p>
     * <p>
     * Relations, which are positive as well as negative inertia, are simply
     * called inertia. Relations, which are neither positive nor negative
     * inertia, are called fluents. The detection of inertia and fluents is easy
     * because in ADL, effects are restricted to conjunctions of literals.
     * Furthermore, this information can be obtained with a single pass over the
     * domain description, which takes almost no time at all.
     * </p>
     * <p>
     * Note: before calling this method the domain must be encode into integer
     * and the negation must be move inward the expression.
     * </p>
     *
    ...
    {
        Encoder.tableOfInferredDomains = new ArrayList<>(
                Encoder.tableOfPredicates.size());
    ...
    /**
     * This method creates the predicates predicatesTables used to simplify
     * atomic expression.
     *
     * @param init the initial state.
     */
    static void createPredicatesTables(final Set<IntExp> init)
    {
        ...
        for (final IntExp fact : init)
        {
            final int arity = Encoder.tableOfTypedPredicates
                    .get(fact.getPredicate()).size();
            final List<IntMatrix> pTables = Encoder.predicatesTables
                    .get(fact.getPredicate());
    ...
    /**
     * Return an integer representation of the specified array of integer. For
     * example, if <code>mask = [0, 1, 1]</code> then the method will return 3.
     *
    ...
    void printPredicatesTables(final List<List<IntMatrix>> tables)
    {
        ...
        for (int predicate = 0; predicate < tables.size(); predicate++)
        {
            ...
            final int arity = Encoder.tableOfTypedPredicates.get(predicate)
                    .size();
     ...
     */
    private void print(final int predicate, final int arity, final int[] mask,
            final int[] index, final List<List<IntMatrix>> tables)
    {
        if (index.length == arity)
        {
            ...
            for (int anIndex : index)
            {
                ...
                else
                {
                    realIndexSize++;
                    str.append(" ")
                            .append(Encoder.tableOfConstants.get(anIndex));
                }
            }
            str.append(")");
            ...
            final int counter = tables.get(predicate)
                    .get(PreInstantiation.toInt(mask)).get(realIndex);
            if (counter != 0)
            {
     ...
     */
    static List<IntOp> simplifyOperatorsWithInferedTypes(
            final List<IntOp> operators)
    {
    ...
    private static List<IntOp> simplifyOperatorsWithInferedTypes(final IntOp op)
    {
        ...
        unaryInertia.addAll(
                PreInstantiation.collectUnaryInertia(op.getPreconditions()));
        unaryInertia
                .addAll(PreInstantiation.collectUnaryInertia(op.getEffects()));
        ...
        for (final IntExp inertia : unaryInertia)
        {
            ...
            for (final IntOp o : operators)
            {
                if (o.getArity() > 0)
                {
                    ...
                    // Compute the
                    final String declaredType = Encoder.tableOfTypes
                            .get(dtIndex);
                    final int itIndex = inertia.getPredicate();
                    final String inertiaType = Encoder.tableOfPredicates
                            .get(itIndex);
                    ...
                    if (ti == -1)
                    {
                        ...
                        final Set<Integer> dt1 = new LinkedHashSet<>(
                                Encoder.tableOfDomains.get(dtIndex));
                        dt1.retainAll(
                                Encoder.tableOfInferredDomains.get(itIndex));
                    ...
                    if (ts == -1)
                    {
                        ...
                        final Set<Integer> dt2 = new LinkedHashSet<>(
                                Encoder.tableOfDomains.get(dtIndex));
                        dt2.removeAll(
                                Encoder.tableOfInferredDomains.get(itIndex));
                    ...
                    PreInstantiation.replace(op1.getPreconditions(), inertia,
                            Connective.TRUE, ti, ts);
                    PreInstantiation.replace(op1.getEffects(), inertia,
                            Connective.TRUE, ti, ts);
                    if (!op1.getPreconditions().getConnective()
                            .equals(Connective.FALSE)
                            && !op1.getEffects().getConnective()
                                    .equals(Connective.FALSE))
                    {
                    ...
                    PreInstantiation.replace(op2.getPreconditions(), inertia,
                            Connective.FALSE, ti, ts);
                    PreInstantiation.replace(op2.getEffects(), inertia,
                            Connective.FALSE, ti, ts);

                    if (!op2.getPreconditions().getConnective()
                            .equals(Connective.FALSE)
                            && !op2.getEffects().getConnective()
                                    .equals(Connective.FALSE))
                    {
    ...
    /**
     * Replace all the occurrences of a specified unary inertia contained in the
     * a specified expression by <code>TRUE</code> or <code>FALSE</code>.
     *
     ...
     */
    private static void replace(final IntExp exp, final IntExp inertia,
            final Connective connective, final int ti, final int ts)
    {
        switch (exp.getConnective())
        {
        case ATOM:
        ...
        case EXISTS:
            ...
            {
                PreInstantiation.replace(exp.getChildren().get(0), inertia,
                        connective, ti, ts);
            }
            break;
        case AT_START:
        ...
        case HOLD_DURING:
            PreInstantiation.replace(exp.getChildren().get(0), inertia,
                    connective, ti, ts);
            PreInstantiation.replace(exp.getChildren().get(1), inertia,
                    connective, ti, ts);
            PreInstantiation.replace(exp.getChildren().get(3), inertia,
                    connective, ti, ts);
            break;
        case EQUAL_ATOM:
        case FN_HEAD:
    ...
    private static List<IntExp> collectUnaryInertia(final IntExp exp)
    {
        ...
        case HOLD_AFTER:
        case WHEN:
            unaryInertia.addAll(PreInstantiation
                    .collectUnaryInertia(exp.getChildren().get(0)));
            break;
        case ALWAYS_WITHIN:
        case HOLD_DURING:
            unaryInertia.addAll(PreInstantiation
                    .collectUnaryInertia(exp.getChildren().get(0)));
            unaryInertia.addAll(PreInstantiation
                    .collectUnaryInertia(exp.getChildren().get(1)));
            unaryInertia.addAll(PreInstantiation
                    .collectUnaryInertia(exp.getChildren().get(3)));
            break;
        case EQUAL_ATOM:
        case FN_HEAD:
        case FN_ATOM:
