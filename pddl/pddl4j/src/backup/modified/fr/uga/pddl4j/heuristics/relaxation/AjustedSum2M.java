...
/**
 * This class implement the adjusted sum 2M heuristic. This heuristic improves
 * the adjusted sum 2 heuristic by replacing the computation of the interaction
 * degree of the of the adjusted sum 2 heuristic. Now, we have the following
 * heuristic:
 * 
 * <pre>
 ...
 * <li><tt>delta(S) := max(lev({p, q} - max{levl(p), level(q)})</tt>for all
 * <tt>p</tt> in <tt>S</tt></li>
 * </ul>
 * <p>
 * Note that computing <tt>delta(S)</tt> is equivalent to compute the set level
 * heuristic.
 * </p>
...
public final class AjustedSum2M extends RelaxedGraphHeuristic
{
    ...
    /**
     * Creates a new <code>AJUSTED_SUM2M</code> heuristic for a specified
     * planning problem.
     *
    ...
    {
        super.setGoal(goal);
        // First, we expand the relaxed planing graph to compute the relaxed
        // plan value heuristic
        ...
        return super.isGoalReachable() ? super.getRelaxedPlanValue()
                + (this.delta.estimate(state, goal) - super.getMaxValue())
                : Integer.MAX_VALUE;
    }
