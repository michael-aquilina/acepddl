...
/**
 * This abstract class implements the basic methods used by all heuristics based
 * on the computation of a planning graph with mutual exclusions.
 * <p>
 * This implementation is based on the implementation of the STAN planner
 * proposed D. Long and M. Fox. To have more information about this
 * implementation see D. Long, M. Fox (1999). Efficient Implementation of the
 * Plan Graph in STAN. Journal of Artificial Intelligence Research,
 * 10(1):87-115.
 * </p>
 * 
...
public abstract class GraphHeuristic extends AbstractHeuristic
{
    ...
    /**
     * The list of operators mutual exclusions of the planning graph stored by
     * level.
     */
    private List<BitMatrix> operatorsMutex;

    /**
     * The list of propositions mutual exclusions of the planning graph stored
     * by level.
     */
    ...
    /**
     * The bit vector that represents the propositions layer of the planning
     * graph.
     */
    ...
    /**
     * The array that contains the string representation of the propositions.
     * This array can be used only if the debug flag is true.
     */
    private String[] propositions;

    /**
     * The array that contains the string representation of the operators. This
     * array can be used only if the debug flag is true.
     */
    private String[] operators;

    /**
     * The flag used to indicate if the goal is reached after the expansion of
     * the planning graph.
     */
    private boolean goalReached;

    /**
     * The flag used to indicate if the planning graph has reached its level
     * off.
     */
    private boolean levelOff;

    /**
     * The list that contains for each level and for each proposition which
     * operators produce it.
     */
    ...
    /**
     * The bit vector used to store the new operators during the planning graph
     * expansion.
     */
...
//
//        // If debug flag is true we compute the string representation of the
//        // problem propositions
//        if (this.debug)
//        {
//            // The array that contains string representation of the problem
//            // propositions
...
//                this.propositions[i + this.negOffset] = "(not "
//                        + this.propositions[i] + ")";
//            }
//        }
//
...
//        final List<ACEPlannerActionModel> pbOperators = problem.getOperators();
//        for (ACEPlannerActionModel op : pbOperators)
//        {
//            this.nbOperators += op.getEffects().size();// op.getCondEffects().size();
//        }
//
//        // If debug flag is true we create the array that contains the string
...
//        }
//
//        // Initialize the two arrays that wil contain the preconditions and the
//        // effects of the
//        // unconditional operators of the planning problem
...
//            if (debug)
//            {
//                this.operators[negI] = "noop(not ("
//                        + problem.toString(proposition) + "))";
//            }
...
//        for (final ACEPlannerActionModel op : pbOperators)
//        {
//            final List<ACEPlannerEffectTuple> condEffects = op.getEffects();// op.getCondEffects();
//            // For each conditional effect we create a new operator
//            for (int ceIndex = 0; ceIndex < condEffects.size(); ceIndex++)
//            {
//                final ACEPlannerEffectTuple cEffect = condEffects.get(ceIndex);
//                if (debug)
//                {
//                    this.operators[uncondOpIndex] = "("
//                            + problem.toShortString(op) + ")_" + ceIndex;
//                }
//                final BitVector precond = new BitVector();
//                precond.or(new BitSet(10));// op.getConditions().getPositive());
//                precond.or(new BitSet(10));//cEffect.getCondition().getPositive());
//                BitVector neg = new BitVector(10);// op.getPreconditions().getNegative();
//                for (int p = neg.nextSetBit(0); p >= 0; p = neg
//                        .nextSetBit(p + 1))
//                {
//                    precond.set(p + this.negOffset);
//                }
//                neg = new BitVector(10);// cEffect.getCondition().getNegative();
//                for (int p = neg.nextSetBit(0); p >= 0; p = neg
//                        .nextSetBit(p + 1))
//                {
//                    precond.set(p + this.negOffset);
...
//        }
//
//        // Initialize the array that must contain the level of the positive
//        // propsLayer
...
//        // The bit vector that is used to store the new operators during
//        // expansion
//        this.newOperators = new BitVector();
    ...
    protected final int expandPlanningGraph(final BitState state)
    {
        ...
        // The array that contains the level of the positive proposition
        // apparition
        ...
        for (int p = this.propsLayer.nextSetBit(0); p >= 0; p = this.propsLayer
                .nextSetBit(p + 1))
        {
        ...
        // Initialize the boolean flag used to indicate if the goal is reached
        // to false
        this.goalReached = false;
        // Initialize the boolean flag used to indicate if level of the graph is
        // off to false
        this.levelOff = false;
        ...
        {
            this.levelOff = true;
            // Update the achiever of the level by adding achiever at previous
            // level
            final BitMatrix lAchievers = new BitMatrix(this.nbPropositions,
                    this.nbOperators);
            ...
            // Clear the bit vector that will contain the operator to add at the
            // next level
            this.newOperators.clear();
            // Initialize the bit vector that will contain the propositions to
            // add at the next level
            ...
            for (int op = this.opsLayer
                    .nextClearBit(this.nbPropositions); op > 0
                            && op < this.nbOperators; op = this.opsLayer
                                    .nextClearBit(op + 1))
            {
                        ...
                        && this.isMutexFree(this.preconditions[op], k))
                {
                    ...
                    for (int p = effs.nextSetBit(0); p >= 0; p = effs
                            .nextSetBit(p + 1))
                    {
                        lAchievers.set(p, op);
                    }
            ...
            // Add the new effects of the applicable operator to the
            // propositions layer
            this.propsLayer.or(newPropositions);
            // Update the level of the new propositions
            for (int p = newPropositions.nextSetBit(
                    0); p >= 0; p = newPropositions.nextSetBit(p + 1))
            {
            ...
            this.goalReached = this.propsLayer.include(this.bvgoal)
                    && this.isMutexFree(this.bvgoal, k);
        }
        return k;
    }

    /**
     * Updates the propositions mutex at a specified level of the planning
     * graph.
     *
     * @param lev the level.
     */
    private void updatePropositionsMutex(final int lev)
    {
        ...
        for (int pi = this.propsLayer.nextSetBit(
                0); pi >= 0; pi = this.propsLayer.nextSetBit(pi + 1))
        {
            for (int pj = this.propsLayer.nextSetBit(
                    0); pj >= 0; pj = this.propsLayer.nextSetBit(pj + 1))
            {
    ...
    private void updateOperatorsMutex(final int lev)
    {
        if (lev > 1)
        {
            ...
            for (int oi = this.opsLayer.nextSetBit(
                    0); oi >= 0; oi = this.opsLayer.nextSetBit(oi + 1))
            {
                for (int oj = this.opsLayer.nextSetBit(
                        0); oj >= 0; oj = this.opsLayer.nextSetBit(oj + 1))
                {
                    if (oi > oj && omk1.get(oi, oj)
                            && this.areOperatorsMutex(oi, oj, lev))
                    {
                        omk.set(oi, oj);
                        omk.set(oj, oi);
                    }
            ...
            for (int oi = this.newOperators.nextSetBit(
                    0); oi >= 0; oi = this.newOperators.nextSetBit(oi + 1))
            {
                for (int oj = this.opsLayer.nextSetBit(
                        0); oj >= 0; oj = this.opsLayer.nextSetBit(oj + 1))
                {
    ...
    {
        int value = 0;
        for (int p = this.bvgoal.nextSetBit(0); p >= 0; p = this.bvgoal
                .nextSetBit(p + 1))
        {
    ...
    /**
     * Returns <code>true</code> if two operator are mutex at a specified level.
     * Two opsLayer are mutex if the opsLayer are dependent or an operator has a
     * mutex precondition at level lev;
     *
     ...
     * mutex in the previous level lev - 1.
     *
     ...
     *         <code>false</code> otherwise.
     */
    private boolean arePropositionsMutex(final int pi, final int pj,
            final int lev)
    {
    ...
    /**
     * Returns <code>true</code> if a set of facts is mutex free in a specified
     * proposition level of the planning graph.
     *
     * @param state the facts to be tested.
     * @param level the level of the graph at which the test is done.
     * @return <code>true</code> if a set of facts is mutex free in a specified
     *         proposition level of the planning graph, <code>false</code>
     *         otherwise.
     */
     ...
     * @return <code>true</code> if two states are consistent;
     *         <code>false</code> otherwise.
     */
    private boolean areConsistantStates(BitVector s1, BitVector s2)
    {
        ...
        for (int p = state.nextSetBit(0); p >= 0
                && p < offset; p = state.nextSetBit(p + 1))
        {
    ...
    /**
     * Return <code>true</code> if the goal is reached in the last proposition
     * level of the expanded planning graph.
     *
     * @return <code>true</code> if the goal is reached in the last proposition
     *         level of the expanded planning graph; <code>false</code>
     *         otherwise.
     */
