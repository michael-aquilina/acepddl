...
/**
 * This class is used to to parse in the atomic formula skeleton and atomic
 * function skeleton.
 *
 * @author D. Pellier
...
public class NamedTypedList implements Serializable
{
    ...
    /**
     * The list of the types of this name typed list. The list of types is used
     * to encode function type since PDDL 3.1.
     */
    ...
    public NamedTypedList(final NamedTypedList list)
    {
        ...
        this.arguments.addAll(list.getArguments().stream().map(TypedSymbol::new)
                .collect(Collectors.toList()));
        this.types.addAll(list.types.stream().map(Symbol::new)
                .collect(Collectors.toList()));
    }
     ...
     * @return <tt>true</tt> if this named typed list is equal to <tt>obj</tt>,
     *         i.e., <tt>other</tt> is not null and of type
     *         <tt>NamedTypedList</tt> and it has the same name and the same
     *         list of arguments and types; otherwise it returns <tt>false</tt>.
    ...
    public int hashCode()
    {
        return this.getName().hashCode() + this.arguments.hashCode()
                + this.types.hashCode();
    }
    ...
    /**
     * Renames the variable contained in this typed list. For instance, if the
     * nth argument is a variable it will be rename <code>?Xn</code>.
     *
        ...
        if (!this.types.isEmpty())
        {
            ...
            else if (this.types.size() == 2)
            {
                ...
                this.types.stream().filter(type -> !type.equals(Parser.OBJECT))
                        .forEach(type -> str.append(" ").append(
                                type.toString().toUpperCase(Locale.ENGLISH)));
                str.append(")");
            }
        }
        return str.toString();
