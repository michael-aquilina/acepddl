...
/**
 * <p>
 * This class contains methods to encode operators, goal and initial state into
 * <code>BitSet</code> representation.
 * </p>
 * <p>
 * Before encoding, every expression is normalized, i.e., put in disjunctive
 * normal form (DNF) for preconditions and put in conjunctive normal form (CNF)
 * for effects. If an operator has disjunctive preconditions, a new operator is
 * created such all operators after normalization have only conjunctive
 * precondition.
 * </p>
 * <p>
 * <b>Notes:</b>
 * <ul>
 * <li>the problem of converting a well form formula of the first order-logic
 * into DNF of CNF is exponential.</li>
 * <li>The method does not compute the shorter DNF or CNF formula. If you want
 * it have a look to the Quine and McCluskey algorithm.</li>
 * </ul>
 * </p>
 *
 * @author D. Pellier
...
final class BitEncoding
{
    ...
    /**
     * Encode a list of specified operators into <code>BitSet</code>
     * representation. The specified map is used to speed-up the search by
     * mapping the an expression to this index.
     *
     ...
     */
    static List<ACEPlannerBitOp> encodeOperators(final List<IntOp> operators,
            final Map<IntExp, Integer> map) throws UnexpectedExpressionException
    {

        // Normalize the operators
        BitEncoding.normalize(operators);

        final List<ACEPlannerBitOp> ops = new ArrayList<>(operators.size());
        for (IntOp op : operators)
        {
            final int arity = op.getArity();
            final ACEPlannerBitOp bOp = new ACEPlannerBitOp(op.getName(), arity);
            ...
            bOp.setPreconditions(
                    BitEncoding.encode(op.getPreconditions(), map));
            ...
            for (IntExp ei : effects)
            {
                ...
                if (connective.equals(Connective.WHEN))
                {
                    ...
                    condBitExp.setCondition(
                            BitEncoding.encode(children.get(0), map));
                    condBitExp.setEffects(
                            BitEncoding.encode(children.get(1), map));
     ...
     * specified map is used to speed-up the search by mapping the an expression
     * to this index.
     *
     * @param goal the goal to encode.
     * @param map the map that associates to a specified expression its index.
     * @return a list of <code>BitExp</code> that represents the goal as a
     *         disjunction of <code>BitExp</code>.
     */
            ...
            throws UnexpectedExpressionException
    {
        ...
        if (Encoder.codedGoal.size() > 1)
        {
            ...
            {
                final ACEPlannerBitOp op = new ACEPlannerBitOp(Constants.DUMMY_OPERATOR, 0);
                op.setDummy(true);
    ...
    /**
     * Encode a specified initial state in it <code>BitExp</code>
     * representation. The specified map is used to speed-up the search by
     * mapping the an expression to this index.
     *
     ...
     */
    static BitExp encodeInit(final Set<IntExp> init,
            final Map<IntExp, Integer> map)
    {
    ...
    /**
     * Encode an specified <code>IntExp</code> in its <code>BitExp</code>
     * representation.The specified map is used to speed-up the search by
     * mapping the an expression to this index.
     *
     ...
     */
    private static BitExp encode(final IntExp exp,
            final Map<IntExp, Integer> map) throws UnexpectedExpressionException
    {
    ...
    /**
     * Normalize the operators, i.e, put in disjunctive normal form (DNF) for
     * preconditions and put in conjunctive normal form (CNF) for effects. If an
     * operator has disjunctive preconditions, a new operator is created such
     * all operators after normalization have only conjunctive precondition.
     *
        ...
        case AND:
            ...
            for (int i = 1; i < exp.getChildren().size(); i++)
            {
                ...
                for (IntExp newAnd : dnf.getChildren())
                {
                    ...
                    {
                        ek.getChildren().stream().filter(
                                el -> !newAnd.getChildren().contains(el))
                                .forEach(el -> {
