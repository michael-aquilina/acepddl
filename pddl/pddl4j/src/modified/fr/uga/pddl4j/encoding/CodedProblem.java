...
/**
 * This class implements a problem where operators are instantiated and encoded
 * in <code>BitSet</code> representation.
 *
 * @author D. Pellier
...
public class CodedProblem
{
     ...
     */
    private List<ACEPlannerBitOp> operators;
    ...
    /**
     * The default constructor with a private package access to prevent instance
     * creation.
     */
    CodedProblem()
    {
    ...
    public CodedProblem(CodedProblem other)
    {
        ...
        {
            final Set<Integer> copy = si.stream()
                    .collect(Collectors.toCollection(LinkedHashSet::new));
            this.domains.add(copy);
        }
        this.constants = new ArrayList<>();
        this.constants
                .addAll(other.constants.stream().collect(Collectors.toList()));
        this.predicates = new ArrayList<>();
        this.predicates
                .addAll(other.predicates.stream().collect(Collectors.toList()));
        ...
        this.functions
                .addAll(other.functions.stream().collect(Collectors.toList()));
        ...
        this.inertia
                .addAll(other.inertia.stream().collect(Collectors.toList()));
        this.relevantFacts = new ArrayList<>();
        this.relevantFacts.addAll(other.relevantFacts.stream().map(IntExp::new)
                .collect(Collectors.toList()));
        this.operators = new ArrayList<>();
        this.operators.addAll(other.operators.stream().map(ACEPlannerBitOp::new)
                .collect(Collectors.toList()));
    ...
    /**
     * Returns the table of domains for each type of the problem. TODO to
     * remove: never used.
     * 
    ...
    /**
     * Returns the signatures of the predicates defined in the problem. TODO to
     * remove: never used.
     * 
     ...
     *
     * @param signatures the signatures of the predicates defined in the
     *            problem.
     */
    ...
    /**
     * Returns the signatures of the functions defined in the problem. TODO to
     * remove: never used.
     * 
     ...
     */
    public final List<ACEPlannerBitOp> getOperators()
    {
        return operators;
    }
     ...
     */
    final void setOperators(final List<ACEPlannerBitOp> operators)
    {
    ...
    /**
     * Returns <code>true</code> if this problem is solvable, i.e., if its goal
     * was not simplified to FALSE during the encoding.
     *
     * @return <code>true</code> if this problem is solvable;
     *         <code>false</code>.
     */
     ...
     */
    public final String toShortString(final ACEPlannerBitOp op)
    {
     ...
     * @return a string representation of the specified operator.
     */
    public final String toString(final ACEPlannerBitOp op)
    {
     ...
     *
     * @param exp the expression.
     * @param sep the string separator used between the predicate symbol and the
     *            arguments.
     * @return a string representation of the specified expression.
     */
    ...
    public final String toString(final Plan plan)
    {
        ...
        {
            for (ACEPlannerBitOp a : plan.getActionSet(t))
            {
        ...
        final int timeSpecifierSize = (int) Math
                .log10(plan.timeSpecifiers().size()) + 1;

        final StringBuilder str = new StringBuilder();
        plan.timeSpecifiers().forEach(time -> plan.getActionSet(time)
                .forEach(a -> str.append(String.format(
                        "%0" + timeSpecifierSize + "d: (%" + actionSize
                                + "s) [%d]%n",
                        time, this.toShortString(a),
                        ((int) a.getDuration())))));
        return str.toString();
    }
}
