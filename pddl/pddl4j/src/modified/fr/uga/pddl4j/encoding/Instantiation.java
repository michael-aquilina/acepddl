...
final class Instantiation
{
    ...
    static List<IntOp> instantiateOperators(final List<IntOp> operators)
    {
        ...
        {
            // If an operator has a parameter with a empty domain the operator
            // must be removed
            boolean toInstantiate = true;
            int i = 0;
            while (i < op.getArity() && toInstantiate)
            {
                toInstantiate = !Encoder.tableOfDomains
                        .get(op.getTypeOfParameters(i)).isEmpty();
                i++;
            }
            if (toInstantiate)
            {
     ...
     * @return the list of operators instantiated corresponding the specified
     *         operator.
     */
    static List<IntOp> instantiate(final IntOp operator, final int bound)
    {
        ...
        if (!operator.getPreconditions().getConnective()
     ...
     * Instantiates a specified operator.
     * <p>
     * The assumption is made that different operator parameters are
     * instantiated with different constants, i.e., the planner never generates
     * actions like move(a,a) because we consider this as a bad domain
     * representation that should be revised. In fact, in operators with
     * identical constant parameters, all but one of the constants are
     * superfluous and can be skipped from the representation without loss of
     * information.
     * </p>
     *
     ...
     * @see IntOp
     */
    private static void instantiate(final IntOp op, final int index,
            final int bound, final List<IntOp> operators)
    {
        ...
        {
            final Set<Integer> values = Encoder.tableOfDomains
                    .get(op.getTypeOfParameters(index));
                ...
                if (!op.isAlreadyInstantiatedWith(value))
                {
                    ...
                    if (!precond.getConnective().equals(Connective.FALSE))
                    {
                        ...
                        if (!effects.getConnective().equals(Connective.FALSE))
                        {
                            ...
                            {
                                copy.setTypeOfParameter(i,
                                        op.getTypeOfParameters(i));
                            }
                            for (int i = 0; i < index; i++)
                            {
                                copy.setValueOfParameter(i,
                                        op.getValueOfParameter(i));
                            }
                            copy.setValueOfParameter(index, value);
                            Instantiation.instantiate(copy, index + 1, bound,
                                    operators);
                        }
                    }
                }
    ...
    static void expandQuantifiedExpression(final IntExp exp)
    {
        ...
        case AND:
            ...
            while (i.hasNext() && exp.getConnective().equals(Connective.AND))
            {
                ...
                // Remove quantified expression where the domain of the
                // quantified variable is empty
        ...
        case OR:
            ...
            while (i.hasNext() && exp.getConnective().equals(Connective.OR))
            {
                ...
                Instantiation.expandQuantifiedExpression(ei);
                // If a child expression is TRUE, the whole disjunction becomes
                // TRUE.
    ...
    /**
     * This method simplify a specified expression. The rules of simplification
     * are as below:
     * <ul>
    ...
    private static void simplify(final IntExp exp)
    {
        ...
        case EQUAL_ATOM:
            ...
            if (arg1 == arg2)
            {
                // The equality is TRUE: arg1 and arg2 are the same variable or
                // the same constant
                exp.setConnective(Connective.TRUE);
            }
        ...
        case AND:
                    ...
                    && exp.getConnective().equals(Connective.AND))
            {
                ...
                if (ei.getConnective().equals(Connective.FALSE))
                {
                    // If a child expression is FALSE, the whole conjunction
                    // becomes FALSE.
                    exp.setConnective(Connective.FALSE);
                }
                ...
                else if (ei.getConnective().equals(Connective.AND))
                {
                    ...
                    exp.getChildren().remove(i); // removing the inner
                                                 // conjunction.
                    int j = 0;
                    int added = 0;
    ...
    /**
     * Substitutes all occurrence of a specified variable into an expression by
     * a constant.
     *
     * @param exp the expression.
     ...
     */
    private static void substitute(final IntExp exp, final int var,
            final int cons)
    {
        switch (exp.getConnective())
        {
    ...
    /**
     * This method simplifies an atomic specified expression. Two cased must be
     * considered:
     * <ul>
     * <li>1. If the expression is a positive inertia and the number of unifying
     * ground instances of the specified expression that are contained in the
     * initial state is equal to 0 then the expression is simplified to
     * FALSE.</li>
     * <li>2. If the expression is a negative inertia and then the number of all
     * possible type-consistent ground instances of the specified expression
     * then the expression is simplified to TRUE.
     * </ul>
     *
    ...
    private static void simplyAtom(final IntExp exp)
    {
        ...
        // Compute the mask i.e., the vector used to indicate where the constant
        // are located in the
        // atomic expression.
        int indexSize = 0;
        ...
        // Compute the index to access to the predicates table and compute the
        // product (max) of the
        // tableOfDomains of the non instantiated arguments of the atomic
        // expression.
        int j = 0;
        int max = 1;
        final int[] index = new int[indexSize];
        final List<Integer> predArg = Encoder.tableOfTypedPredicates
                .get(predicate);
        ...
        // Get the number of unifying ground instances of the specified
        // expression that are
        // contained in the initial state.
        final int n = Encoder.predicatesTables.get(predicate)
                .get(PreInstantiation.toInt(mask)).get(index);
        // CASE 1: If the expression is a positive inertia and the number of
        // unifying ground
        // instances of the specified expression that are contained in the
        // initial state is equal to
        // 0 then the expression is simplified to FALSE.
        final Inertia inertia = Encoder.tableOfInertia.get(predicate);
        if ((inertia.equals(Inertia.POSITIVE)
                || inertia.equals(Inertia.INERTIA)) && n == 0)
        {
            exp.setConnective(Connective.FALSE);
        }
        else if ((inertia.equals(Inertia.NEGATIVE)
                || inertia.equals(Inertia.INERTIA)) && max == n)
        {
            // CASE 2: If the expression is a negative inertia and then the
            // number of all possible
            // type-consistent ground instances of the specified expression then
            // the expression is
            // simplified to TRUE.
            exp.setConnective(Connective.TRUE);
        }
