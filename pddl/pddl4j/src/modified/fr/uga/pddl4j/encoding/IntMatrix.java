...
/**
 * This class implements a matrix at n-dimension. This class is used to store
 * the predicates tables.
 *
 * @author D. Pellier
...
final class IntMatrix implements Serializable
{
     ...
     * @return the integer contained in the matrix at the specified index or
     *         null if no element is at the specified index.
     * @throws ArrayIndexOutOfBoundsException if index.length != dimension and
     *             for all i 0 <= index[i] < size does not hold.
     */
    ...
    /**
     * Return from the a specified index the index in the internal
     * representation.
     *
    ...
    private final int index(final int[] index)
    {
        ...
        {
            acces += index[i - 1]
                    * (int) Math.pow(size, (double) dimension - i);
        }
        return acces;
    }
}
