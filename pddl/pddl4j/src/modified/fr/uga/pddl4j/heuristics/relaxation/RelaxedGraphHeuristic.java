 ...
 * on the computation of a relaxed planning graph ignoring negative effects.
 * This implementation is based on the implementation of the fast forward
 * planner proposed by J. Hoffmann. The relaxed planning computed does not
 * compute mutual exclusion. To have more information about this implementation
 * see Hoffmann, J. and Nebel, B. (2001). The FF Planning System: Fast Plan
...
public abstract class RelaxedGraphHeuristic extends AbstractHeuristic
{
    ...
    /**
     * The array used to store the number of precondition encountered for each
     * operator.
     */
    ...
    /**
     * The array used to store the apparition level of the positive
     * propositions.
     */
    private int[] pPropLevel;

    /**
     * The array used to store the apparition level of the negative
     * propositions.
     */
    ...
    protected RelaxedGraphHeuristic(final CodedProblem problem)
    {
        ...
        final List<ACEPlannerBitOp> operators = problem.getOperators();
        for (ACEPlannerBitOp op : operators)
        {
        ...
        // Initialize the array that must contain the level of the negative
        // propositions
        ...
        // Initialize the array that must contain the difficulty of the
        // operators
        this.operatorsDifficulty = new int[nbUncondOperators];
        // Initialize the array that must contain for each operator the number
        // of its precondition
        // reached
        this.precondCounters = new int[nbUncondOperators];
        // Initialize the array that must contain for each operator its
        // preconditions
        ...
        // The array that contains for each proposition the list of its
        ...
        // The array that contains for each operator the number of propositions
        ...
        {
            final ACEPlannerBitOp op = operators.get(opIndex);
            ...
            for (int ceIndex = 0; ceIndex < condEffects.size(); ceIndex++)
            {
                ...
                for (int p = pPre.nextSetBit(0); p >= 0; p = pPre
                        .nextSetBit(p + 1))
                {
                    this.precondEdges[p].getPositive().set(uncondOpIndex);
                }
                for (int p = nPre.nextSetBit(0); p >= 0; p = nPre
                        .nextSetBit(p + 1))
                {
                ...
                for (int p = pEff.nextSetBit(0); p >= 0; p = pEff
                        .nextSetBit(p + 1))
                {
                    this.effectsEdges[p].getPositive().set(uncondOpIndex);
                }
                for (int p = nEff.nextSetBit(0); p >= 0; p = nEff
                        .nextSetBit(p + 1))
                {
                ...
                // We initialize the number of precondition of the unconditional
                // operator
    ...
    /**
     * Set the goal of the the relaxed problem to solve in order to compute the
     * heuristic.
     *
     * @param goal the goal.
    ...
    protected final int expandRelaxedPlanningGraph(final BitState state)
    {
        ...
        // The array that contains the level of the negative proposition
        // apparition
        Arrays.fill(this.nPropLevel, Integer.MAX_VALUE);
        // The array that contains the counter of precondition encounter for
        // each operator
        ...
        // The positive accumulator used to store the set of positive
        // proposition already reached
        final BitVector pAcc = new BitVector();
        // The negative accumulator used to store the set of negative
        // proposition already reached
        final BitVector nAcc = new BitVector();

        // We start building the relaxed planning graph
        // The graph is expanded until the goal and the fixed point of the graph
        // is not reached
        while (this.goalCounter != this.goalCardinality
                && (!ppk.isEmpty() || !npk.isEmpty()))
        {
            ...
            for (int p = ppk.nextSetBit(0); p >= 0; p = ppk.nextSetBit(p + 1))
            {
                // We getActionSet the operator that have this positive
                // proposition as precondition
                ...
                for (int pe = pEdges.nextSetBit(0); pe >= 0; pe = pEdges
                        .nextSetBit(pe + 1))
                {
                    // If the operator has a no-empty set of preconditions we
                    // increment its counter
                    if (this.precondCardinality[pe] != 0)
                    {
                        this.precondCounters[pe]++;
                    }
                    // Finally, if the all the preconditions of an operator hold
                    // we mark the
            ...
            for (int p = npk.nextSetBit(0); p >= 0; p = npk.nextSetBit(p + 1))
            {
                ...
                nAcc.set(p);
                // We update the counter associated to the operator precondition
                for (int pe = nEdges.nextSetBit(0); pe >= 0; pe = nEdges
                        .nextSetBit(pe + 1))
                {
                    // If the operator has a no-empty set of preconditions we
                    // increment its counter
            ...
            // The bit vector used to the store the new positive proposition at
            // the next level
            final BitVector pNewProps = new BitVector();
            // The bit vector used to the store the new negative proposition at
            // the next level
            final BitVector nNewProps = new BitVector();
            // For each new operator at level k
            for (int o = newOps.nextSetBit(0); o >= 0; o = newOps
                    .nextSetBit(o + 1))
            {
                // We mark o as appearing at the level k
                this.operatorsLevel[o] = this.level;
                // We accumulate the positive effects of o for the next
                // proposition level k
                pNewProps.or(this.effects[o].getPositive());
                // We accumulate the negative effects of o for the next
                // proposition level k
                nNewProps.or(this.effects[o].getNegative());
                // Then we compute the difficulty of operator as the sum of the
                // level of their
                // preconditions
            ...
            // Now, we compute the new proposition level just by adding positive
            // and negative
            ...
            for (int p = ppk.nextSetBit(0); p >= 0; p = ppk.nextSetBit(p + 1))
            {
                ...
                // Update the goal counter if a positive goal proposition is
                // reached
                if (pGoal.get(p))
                {
                    this.goalCounter++;
            ...
            for (int p = npk.nextSetBit(0); p >= 0; p = npk.nextSetBit(p + 1))
            {
                ...
                // Update the goal counter if a negative goal proposition is
                // reached
                if (nGoal.get(p))
                {
                    this.goalCounter++;
    ...
    /**
     * Returns <code>true</code> if the goal is reachable after the planning
     * graph expansion.
     *
     * @return <code>true</code> if the goal is reachable after the planning
     *         graph expansion; <code>false</code> otherwise.
     */
    ...
    protected final int getRelaxedPlanValue()
    {
        ...
        for (int k = level; k > 0; k--)
        {
            ...
            // Each positive goal at level k we need to find a resolver to
            // support it
            for (int pg = pGk.nextSetBit(0); pg >= 0; pg = pGk
                    .nextSetBit(pg + 1))
            {
                // Select the best resolver according to the difficulty
                // heuristic
                final int resolverIndex = this
                        .select(this.effectsEdges[pg].getPositive(), k);
                if (resolverIndex != -1)
                {
            ...
            // Each negative goal at level k we need to find a resolver to
            // support it
            for (int ng = nGk.nextSetBit(0); ng >= 0; ng = nGk
                    .nextSetBit(ng + 1))
            {
                final int resolverIndex = this
                        .select(this.effectsEdges[ng].getNegative(), k);
                if (resolverIndex != -1)
                {
                    final BitExp pre = this.preconditions[resolverIndex];
                    final BitVector pPre = pre.getPositive();
    ...
    /**
     * Select an effect according to the unconditional operators difficulty
     * heuristic. The question is, which achiever should be choose when no NOOP
     * is available ? It is certainly a good idea to select an achiever whose
     * preconditions seems to be "easy". From the graph building phase, we can
     * obtain a simple measure for the operatorsDifficulty of an action's
     * preconditions as follows:
     * <ul>
     * <li>operatorsDifficulty(o) := SUM_ID(min { i | p is member of the fact
     * layer at time i }) with p in pre(o)</li>
     * </ul>
     * The operatorsDifficulty of each action can be set when it is first
     * inserted into the graph. During plan extraction, facing a fact for which
     * no NOOP is available, we then simply selected an achieving action with
     * minimal operatorsDifficulty. This heuristic works well in situation where
     * there are severals ways to achieve one fact. but some ways need less
     * effort than others.
     *
     * @param resolvers the list of resolver of p.
     * @param lev the level.
     * @return the easier resolver for the proposition <code>p</code> at level
     *         <code>lev</code> or <code>null</code> if a NOOP operator is
     *         available.
     */
    private int select(final BitVector resolvers, final int lev)
    {
        ...
        for (int r = resolvers.nextSetBit(0); r >= 0; r = resolvers
                .nextSetBit(r + 1))
        {
