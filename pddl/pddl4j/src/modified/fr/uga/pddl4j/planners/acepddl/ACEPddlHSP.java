...
package fr.uga.pddl4j.planners.acepddl;
 ...
 */
public final class ACEPlannerHSP extends AbstractPlanner
{

    /**
     * The logger of the class.
     */
    private static final Logger LOGGER = LogManager.getLogger(ACEPlannerHSP.class);
     ...
     */
    public ACEPlannerHSP()
    {
        super();
        this.setHeuristicType(ACEPlannerHSP.DEFAULT_HEURISTIC);
        this.setWeight(ACEPlannerHSP.DEFAULT_WEIGHT);
        this.setSaveState(ACEPlannerHSP.DEFAULT_STATISTICS);
    }
     ...
     *
     * @param heuristicType the heuristicType to use to solved the problem. The
     *            heuristicType cannot be null.
     */
    ...
    public SequentialPlan search(final CodedProblem problem)
    {
        ...
        final Heuristic heuristic = HeuristicToolKit
                .createHeuristic(this.getHeuristicType(), problem);
        ...
        // The list stores the node ordered according to the A* (getFValue = g +
        // h) function
        final PriorityQueue<Node> open = new PriorityQueue<>(100,
                new NodeComparator(currWeight));
        ...
        while (!open.isEmpty() && plan == null && time < timeout)
        {
            ...
            // If the goal is satisfy in the current node then extract the
            ...
            else
            {
                ...
                int index = 0;
                for (ACEPlannerBitOp op : problem.getOperators())
                {
                    ...
                    if (op.isApplicable(current))
                    {
                        ...
                        op.getCondEffects().stream().filter(
                                ce -> current.satisfy(ce.getCondition()))
                                .forEach(ce ->
                        // Apply the effect to the successor node
        ...
        if (isSaveState())
        {
            ...
            this.getStatistics()
                    .setMemoryUsedToSearch(MemoryAgent.deepSizeOf(closeSet)
    ...
    private SequentialPlan extract(final Node node, final CodedProblem problem)
    {
        ...
        {
            final ACEPlannerBitOp op = problem.getOperators().get(n.getOperator());
            plan.add(0, op);
            n = n.getParent();
        }
        return plan;
    ...
    public static String resolveAsJsonPlan(String[] args)
    {
        ...
        try
        {
            ...
            final Properties arguments = ACEPlannerHSP.parseArguments(args);
            final File domain = (File) arguments.get(ACEPlannerHSP.Argument.DOMAIN);
            final File problem = (File) arguments.get(ACEPlannerHSP.Argument.PROBLEM);
            final int traceLevel = 0;
            final int timeout = (Integer) arguments.get(Argument.TIMEOUT);
            final Heuristic.Type heuristicType = (Heuristic.Type) arguments
                    .get(Argument.HEURISTIC);
            final double weight = (Double) arguments.get(ACEPlannerHSP.Argument.WEIGHT);

            // Creates the planner
            final ACEPlannerHSP planner = new ACEPlannerHSP();
            ...
            planner.getStatistics()
                    .setNumberOfActions(pb.getOperators().size());
            planner.getStatistics()
                    .setNumberOfRelevantFluents(pb.getRelevantFacts().size());

            if (!pb.isSolvable())
            {
                StringBuilder strb = new StringBuilder();
                strb.append(String.format(
                        "goal can be simplified to FALSE. no search will solve it%n%n"));
                LOGGER.trace(strb);
    ...
    /**
     * The main method of the <code>HSP</code> example. The command line syntax
     * is as follow:
     * <p>
     * 
     * <pre>
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            final File domain = (File) arguments.get(ACEPlannerHSP.Argument.DOMAIN);
            final File problem = (File) arguments.get(ACEPlannerHSP.Argument.PROBLEM);
            final int traceLevel = (Integer) arguments
                    .get(ACEPlannerHSP.Argument.TRACE_LEVEL);
            final int timeout = (Integer) arguments.get(Argument.TIMEOUT);
            final Heuristic.Type heuristicType = (Heuristic.Type) arguments
                    .get(Argument.HEURISTIC);
            final double weight = (Double) arguments.get(ACEPlannerHSP.Argument.WEIGHT);
            final boolean saveStats = (Boolean) arguments
            ...
            // Creates the problem factory
            final ProblemFactory factory = new ProblemFactory();
            final int factoryTraceLevel = (traceLevel == 8) ? 0
                    : Math.max(0, traceLevel - 1);
            ...
            {
                planner.getStatistics()
                        .setTimeToParse(System.currentTimeMillis() - begin);
            }
            if (!errorManager.isEmpty())
            {
                errorManager.printAll();
            ...
            {
                StringBuilder strb = new StringBuilder();
                strb.append("\nparsing domain file \"").append(domain.getName())
                        .append("\" done successfully")
                        .append("\nparsing problem file \"")
                        .append(problem.getName())
                        .append("\" done successfully").append("\n");
                LOGGER.trace(strb);
            }
            ...
            {
                planner.getStatistics()
                        .setTimeToEncode(System.currentTimeMillis() - begin);
                planner.getStatistics().setMemoryUsedForProblemRepresentation(
                        MemoryAgent.deepSizeOf(pb));
            }
            planner.getStatistics()
                    .setNumberOfActions(pb.getOperators().size());
            planner.getStatistics()
            ...
            if (traceLevel > 0 && traceLevel != 8)
            {
                ...
                strb.append("\nencoding problem done successfully (")
                        .append(planner.getStatistics().getNumberOfActions())
                        .append(" ops, ")
                        .append(planner.getStatistics()
            ...
            // Print the results
            final String problemName = problem.getName().substring(0,
                    problem.getName().indexOf('.'));
            final int numberOfActions = planner.getStatistics()
                    .getNumberOfActions();
            final int numberOfFluents = planner.getStatistics()
                    .getNumberOfRelevantFluents();
            ...
            if (saveStats)
            {
                timeToParseInSeconds = Statistics.millisecondToSecond(
                        planner.getStatistics().getTimeToParse());
                timeToEncodeInSeconds = Statistics.millisecondToSecond(
                        planner.getStatistics().getTimeToEncode());
                timeToSearchInSeconds = Statistics.millisecondToSecond(
                        planner.getStatistics().getTimeToSearch());
                totalTimeInSeconds = timeToParseInSeconds
                        + timeToEncodeInSeconds + timeToSearchInSeconds;
                memoryUsedToSearchInMBytes = Statistics.byteToMByte(
                        planner.getStatistics().getMemoryUsedToSearch());
                memoryForProblemInMBytes = Statistics
                        .byteToMByte(planner.getStatistics()
                                .getMemoryUsedForProblemRepresentation());
                totalMemoryInMBytes = memoryForProblemInMBytes
            ...
            if (traceLevel > 0 && traceLevel != 8)
            {
                ...
                {
                    strb.append(String.format(
                            "%ntime spent:   %8.2f seconds parsing %n",
                            timeToParseInSeconds));
                    strb.append(String.format(
                            "              %8.2f seconds encoding %n",
                            timeToEncodeInSeconds));
                    strb.append(String.format(
                            "              %8.2f seconds searching%n",
                            timeToSearchInSeconds));
                    strb.append(String.format(
                            "              %8.2f seconds total time%n",
                            totalTimeInSeconds));
                    strb.append(String.format(
                            "%nmemory used:  %8.2f MBytes for problem representation%n",
                            memoryForProblemInMBytes));
                    strb.append(String.format(
                            "              %8.2f MBytes for searching%n",
                            memoryUsedToSearchInMBytes));
                    strb.append(String.format(
                            "              %8.2f MBytes total%n%n%n",
                            totalMemoryInMBytes));
                }
                LOGGER.trace(strb);
            }
            else if (traceLevel == 8)
            {
                ...
                {
                    strb.append(String.format(
                            "%5s %8d %8d %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %5d%n",
                            problemName, numberOfActions, numberOfFluents,
                            timeToParseInSeconds, timeToEncodeInSeconds,
                            timeToSearchInSeconds, totalTimeInSeconds,
                            memoryForProblemInMBytes,
                            memoryUsedToSearchInMBytes, totalMemoryInMBytes,
                            plan.size()));
                }
                else
                {
                    strb.append(String.format(
                            "%5s %8d %8d %8.2f %8.2f %8s %8s %8.2f %8s %8s %5s%n",
                            problem.getName(), numberOfActions, numberOfFluents,
                            timeToParseInSeconds, timeToEncodeInSeconds, "--",
                            "--", memoryForProblemInMBytes, "--", "--", "--"));
                }
                LOGGER.trace(strb);
    ...
    {
        final Properties arguments = ACEPlannerHSP.getDefaultArguments();
        try
        {
            for (int i = 0; i < args.length; i += 2)
            {
                if ("-o".equalsIgnoreCase(args[i]) && ((i + 1) < args.length))
                {
                    ...
                    {
                        LOGGER.trace("operators file does not exist: "
                                + args[i + 1] + "\n");
                    }
                        ...
                        && ((i + 1) < args.length))
                {
                    ...
                    if (heuristic == 0)
                    {
                        arguments.put(Argument.HEURISTIC,
                        ...
                        && ((i + 1) < args.length))
                {
                    ...
                    arguments.put(Argument.TRACE_LEVEL, level);
                        ...
                        && ((i + 1) < args.length))
                {
                    final boolean isStatUsed = Boolean
                            .parseBoolean(args[i + 1]);
                    arguments.put(Argument.STATISTICS, isStatUsed);
                }
                else
                {
                    LOGGER.trace("\nUnknown argument for \"" + args[i]
                            + "\" or missing value\n");
                    ACEPlannerHSP.printUsage();
                    throw new FileException("Unknown arguments: " + args[i]);
                }
            }
            if (arguments.get(Argument.DOMAIN) == null
                    || arguments.get(Argument.PROBLEM) == null)
            {
    ...
    private static Properties getDefaultArguments()
    {
        ...
        options.put(ACEPlannerHSP.Argument.HEURISTIC, ACEPlannerHSP.DEFAULT_HEURISTIC);
        options.put(ACEPlannerHSP.Argument.WEIGHT, ACEPlannerHSP.DEFAULT_WEIGHT);
        options.put(ACEPlannerHSP.Argument.TIMEOUT, ACEPlannerHSP.DEFAULT_TIMEOUT * 1000);
        options.put(ACEPlannerHSP.Argument.TRACE_LEVEL, ACEPlannerHSP.DEFAULT_TRACE_LEVEL);
        options.put(Argument.STATISTICS, ACEPlannerHSP.DEFAULT_STATISTICS);
        return options;
    }
    ...
    private static class NodeComparator
            implements Comparator<Node>, Serializable
    {
