...
/**
 * This class implements a compact representation for action based on
 * <code>BitSet</code> structure.
 *
 * @author D. Pellier
...
public class BitOp extends AbstractCodedOp
{
    ...
    /**
     * Creates a new operator from an other. This constructor is the copy
     * constructor.
     *
    ...
    public BitOp(final BitOp other)
    {
        ...
        this.effects.addAll(other.getCondEffects().stream().map(CondBitExp::new)
     ...
     */
    public BitOp(final String name, final int arity, final BitExp preconditions,
            final BitExp effects)
    {
        this(name, arity);
    ...
    /**
     * Returns <code>true</code> if this operators is applicable in a specified
     * state.
     *
     * @param state the state.
     * @return <code>true</code> if this operators is applicable in a specified
    ...
    public BitExp getUnconditionalEffects()
    {
        ...
        this.effects.stream()
                .filter(cEffect -> cEffect.getCondition().isEmpty())
                .forEach(cEffect -> {
