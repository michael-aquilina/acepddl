...
public final class TypedSymbol extends Symbol
{
    ...
    public TypedSymbol(final TypedSymbol symbol)
    {
        ...
        this.types.addAll(symbol.getTypes().stream().map(Symbol::new)
     ...
     * symbol <code>Parser.OBJECT</code> or <code>Parser.NUMBER</code>, the
     * typed list is creates with an empty list of super types.
     *
     * @param symbol the symbol.
     */
    public TypedSymbol(final Symbol symbol)
    {
        super(symbol.getKind(), symbol.getImage(), symbol.getBeginLine(),
                symbol.getBeginColumn(), symbol.getEndLine(),
                symbol.getEndColumn());
        this.types = new ArrayList<>();
    ...
    public String toString()
    {
        ...
        if (!this.equals(Parser.OBJECT) && !this.equals(Parser.NUMBER))
        {
            ...
            else if (this.types.size() == 2)
            {
                ...
                {
                    str.append(" ");
                    str.append(this.types.get(i).toString()
                            .toUpperCase(Locale.ENGLISH));
                }
                str.append(")");
            }
