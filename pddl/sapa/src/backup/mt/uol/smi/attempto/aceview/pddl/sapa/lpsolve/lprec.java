...
package mt.uol.smi.attempto.aceview.pddl.sapa.lpsolve;
...
public class lprec implements constant {
	...
	short names_used; /*
						 * Flag to indecate if names for rows and columns are used
						 */
	...
	int[] col_end; /*
					 * columns_alloc+1 :Cend[i] is the index of the first element after column i.
					 * column[i] is stored in elements col_end[i-1] to col_end[i]-1
					 */
	int[] col_no; /*
					 * mat_alloc :From Row 1 on, col_no contains the column nr. of the nonzero
					 * elements, row by row
					 */
	short row_end_valid; /* true if row_end & col_no are valid */
	int[] row_end; /*
					 * rows_alloc+1 :row_end[i] is the index of the first element in Colno after row
					 * i
					 */
	double[] orig_rh; /*
						 * rows_alloc+1 :The RHS after scaling & sign changing, but before `Bound
						 * transformation'
						 */
	double[] rh; /*
	...
	short[] basis; /*
					 * sum_alloc+1 : basis[i] is TRUE if the column is in the basis
					 */
	short[] lower; /*
					 * "       " :TRUE is the variable is at its lower bound (or in the basis), it
					 * is FALSE if the variable is at its upper bound
					 */
	...
	double[] eta_value; /*
						 ...
						 */
	int[] eta_row_nr; /*
						 * "     " :The Structure containing the Row indexes of Eta
						 */
	int[] eta_col_end; /*
						 * rows_alloc + MaxNumInv : eta_col_end[i] is the start index of the next Eta
						 * column
						 */
	...
	double[] solution; /*
						 * sum_alloc+1 :The Solution of the last LP, 0 = The Optimal Value, 1..rows The
						 * Slacks, rows+1..sum The Variables
						 */
	...
	short[] ch_sign; /*
						 * rows_alloc+1 :TRUE if the Row in the matrix has changed sign (a`x > b, x>=0)
						 * is translated to s + -a`x = -b with x>=0, s>=0)
						 */

	short scaling_used; /* TRUE if scaling is used */
	short columns_scaled; /*
							 * TRUE is the columns are scaled too, Only use if all variables are non-integer
							 */
	double[] scale; /*
					 * sum_alloc+1 :0..Rows the scaling of the Rows, Rows+1..Sum the scaling of the
					 * columns
					 */
