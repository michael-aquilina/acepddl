...
public class solve implements constant {
	...
	/**
	 * BM: Newly added function by Minh B. Do to get the final solution in the form
	 * of array list (values of all variables) so we can interprete to get the final
	 * plan.
	 */
	...
	/*
	 * void write_MPS(lprec lp, FILE *output) { int i, j, marker; double *column;
	 * 
	 * 
	 ...
	 * lp.col_name[i], lp.row_name[j], column[j]); else output.print(
	 * "    var_%-4d  r_%-6d  %g\n", i, j, column[j]); } } for(j=1; j <= lp.rows;
	 * j++) if(column[j] != 0) { if (lp.names_used != FALSE) output.print(
	 * "    %-8s  %-8s  %g\n", lp.col_name[i], lp.row_name[j], column[j]); else
	 * output.print( "    var_%-4d  r_%-6d  %g\n", i, j, column[j]); } } if((marker
	 * % 2) ==1) { output.print(
	 * "    MARK%04d  'MARKER'                 'INTEND'\n", marker); marker++; }
	 * 
	 ...
	 * 
	 * output.print( "BOUNDS\n"); if (lp.names_used != FALSE) for(i = lp.rows + 1; i
	 * <= lp.sum; i++) { if(lp.orig_upbo[i] < lp.infinite) output.print(
		...
		if (Lp.debug != FALSE)
			for (i = Rows + 1; i <= Sum; i++) {
				if (lowbo[i] != 0) {
					print_indent();
					if (Lp.names_used != FALSE)
						System.out.println(Lp.col_name[i - Rows] + " > " + lowbo[i]);
					else
				...
				if (upbo[i] != Infinite) {
					print_indent();
					if (Lp.names_used != FALSE)
						System.out.println(Lp.col_name[i - Rows] + " < " + upbo[i]);
					else
	...
	void invert() {
		...
		if (Lp.print_at_invert != FALSE)
			System.out.print("Start Invert iter " + Lp.iter + " eta_size " + Eta_size + " rhs[0] " + (-Rhs[0]) + " \n");
		...
		for (j = 1; j <= Columns; j++)
			if (fcol[j - 1] != FALSE) {
				...
				while ((row_nr <= Rows) && (frow[row_nr] == FALSE || pcol[row_nr] == FALSE))
					row_nr++; /*
								 * this sometimes sets row_nr to Rows + 1 and makes rhsmincol crash. Solved in
								 * 2.0? MB
								 */
				if (row_nr == Rows + 1)
		...
		if (Lp.print_at_invert != FALSE)
			System.out.print("End Invert                eta_size " + Eta_size + " rhs[0] " + (-Rhs[0]) + "\n");
	...
	private short colprim(Ref colnr, short minit, double[] drow) {
		 ...
		 * System.out.println("drow[] at colprim"); for(i = 1; i <= Sum; i++) // DEBUG {
	...
	private void iteration(int row_nr, int varin, Ref theta, double up, Ref minit, Ref low, short primal,
			double[] pcol) {
		int i, k, varout;
		double f;
		double pivot;
		...
		if (Lp.trace != FALSE) {
			...
			if (minit.value != 0) {
				if (Lower[varin] == FALSE)
					...
					System.out.print("Iteration:" + Lp.iter + ", variable" + varin + " changed its upper bound of "
							+ Upbo[varin] + " to 0\n");
			} else
				System.out.print(
