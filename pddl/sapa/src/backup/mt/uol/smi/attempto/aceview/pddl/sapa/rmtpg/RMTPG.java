...
public class RMTPG {
				...
				for (j = 0; j < k; j++) {
					...
					if ((costPropOption == 0) || (costPropOption == 2)) {
						...
						for (l = 0; l < actLevel[actID.intValue()].numPrecond(); l++) {
							...
							if ((factID != effectID) && (factLevel[factID].isIn()) && (factLevel[factID].getCost(-1) > factCost))
								break;
						}
	 ...
	 * @param ctime
	 */
	public int costPropagation(GPredDB currentState, ArrayList events, float ctime, ArrayList pendingCond, ArrayList pendingCondTime) {
