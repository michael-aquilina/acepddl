...
import ch.uzh.ifi.attempto.aceview.OWLXMLTransformer;
import ch.uzh.ifi.attempto.aceview.ParserHolder;
...
public class ACESnippetImpl_Planner implements ACESnippet {

	private static final Logger logger = Logger.getLogger(ACESnippetImpl_Planner.class);
	...
	/**
	 * <p>
	 * Constructs an ACE snippet from a list of ACE sentences.
	 * </p>
	 * 
	 * @param ns
	 *            Default namespace of the snippet
	 * @param sentences
	 *            List of sentences that the snippet contains
	 */
	public ACESnippetImpl_Planner(OWLOntologyID ns, List<ACESentence> sentences, OWLModelManager modelManager,TokenMapper aceLexicon) {
		if (sentences == null) {
	...
	/**
	 * <p>
	 * Returns a set of contentword forms of this snippet.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Parses the text of the snippet with the Manchester OWL Syntax parser; if this
	 * fails then parses with an ACE parser. In the later case, also optionally
	 * paraphrases the snippet.</li>
	 */
	private void init(OWLModelManager modelManager) {
		...
		if (prefs.getUseMos() && sentences.size() == 1) {
			...
			try {
				...
				logger.trace("Parsing with the MOS parser: " + sentences.iterator().next());
				mosAxiom = parseWithMos(sentences.iterator().next(), base, modelManager);
				logger.trace(mosAxiom);
		...
		if (!prefs.getParseWithUndefinedTokens()) {
			...
			for (String wordFrom : contentWordForms) {
				if (!aceLexicon.containsWordform(wordFrom)) {
					messages.add(
							new Message("error", "token", null, null, wordFrom, "Add this wordform to the lexicon"));
					logger.error("Add " + wordFrom +" to the lexicon");
				}
			}
		}

		// In case there are no lexical entries for at least one token, and the
		// preferences
	...
	private void parseWithAceParser(ACEViewPreferences prefs, TokenMapper aceLexicon, Set<String> contentWordforms) throws OWLOntologyCreationException {
		logger.trace("Wordforms: " + contentWordforms);
		Lexicon lexicon = aceLexicon.createLexicon(contentWordforms);
		if (lexicon.getEntries().isEmpty()) {
			logger.error("Parsing with empty lexicon.");
			lexicon = null;
		} else {
			logger.trace("Parsing with lexicon:\n" + lexicon.toString());
		}
		...
		if (result != null) {
			...
			if (errorMessages.isEmpty()) {
				...
				OWLOntologyManager manager = OWLManager.createOWLOntologyManager(); // ACETextManager.getInstance().createOWLOntologyManager();
	...
	public OWLLogicalAxiom parseWithMos(ACESentence sentence, String base, OWLModelManager modelManager) {
		// Remove the last token (a dot or a question mark) of the given sentence.
		String mosStr = sentence.toMOSString();
		OWLModelManager mngr = modelManager;
		return OntologyUtils.parseWithMosParser(mngr.getOWLOntologyManager(), mngr.getActiveOntology(),
				mngr.getOWLDataFactory(), new ProtegeOWLEntityChecker(mngr.getOWLEntityFinder()), base, mosStr,
				sentence.isQuestion());
	}
	...
	/**
	 * <p>
	 * Returns <code>true</code> iff this snippet contains errors, either ACE syntax
	 * errors or errors that were encountered when mapping the (legal ACE snippet)
	 * into OWL/SWRL.
	 * </p>
	 * 
	 * <p>
	 * Iff this method returns <code>true</code>, then {@link #getLogicalAxioms()}
	...
	/**
	 * <p>
	 * Extracts the IRI from the OntologyID and converts it into a string. If the
	 * IRI is missing the returns an empty string.
	 * </p>
	 * 
	 * <p>
	 * TODO: think about it
	...
	private String getOntologyIRIAsString() {
		Optional<IRI> ontologyIRI = ns.getOntologyIRI();
		if (ontologyIRI == null || ontologyIRI.get() == null) {
			return "";
		}
	...
	/**
	 * <p>
	 * Note: This rendering is the basis of equals and hashCode.
	 * </p>
	 * 
	...
	@Override
	public boolean isEqual(ACESnippet snippet) {
		if (snippet == null) {
			return false;
		}
		if (snippet.makeStringID() == null && this.makeStringID() == null) {
			return true;
		}
		if (snippet.makeStringID() == null || this.makeStringID() == null) {
			return false;
		}
		return snippet.makeStringID().equals(this.makeStringID());
	}
